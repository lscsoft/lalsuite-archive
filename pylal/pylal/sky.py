    ...
    for n,row in enumerate(self):
      ...
      for i in xrange(len(ifos)):
        for j in xrange(i+1, len(ifos)):
          timeDelays[n].append(date.XLALArrivalTimeDiff(detectors[i].location,\
    ...
    if skyPoint.system == 'horizon':
      ...
      if system == 'horizon':
        if zenith.system == 'geographic':
          ...
          raise AttibuteError("Cannot convert from geographic to horizon with"+\
                              " zenith point not in geogrphic of equatorial "+\
                              "systems.")
      else:
        skyPoint = GeographicToEquatorial(skyPoint, gpstime)
...
def HorizonToSystem(input, zenith):
  ...
  if input.system.lower() != 'horizon':
    raise AttributeError("Cannot convert from horizon for point in system='%s'"\
                         % input.system.lower())

  if zenith.system.lower() != 'equatorial'\
  and zenith.system.lower() != 'geographic':
    raise AttributeError("zenith must have coordinate system = 'equatorial' "+\
                         "or 'geographic'")
...
def parse_sites(ifos):
  ...
  ifos2 = []
  for ifo in ifos:
    if len([i for i in ifos2 if i.startswith(ifo[0])]) == 0: ifos2.append(ifo)
