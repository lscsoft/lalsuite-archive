...
def create_vetoed_sngl_table(connection, veto_segments_name, temporary = True):
	"""
	Create the string_vetoed_sngl table in the database at @connection
	containing the event IDs of sngl_burst rows vetoed by the veto
	segments named @veto_segments_name.  If @temporary is True the
	table is created as a temporary table, otherwise it is not.  If a
	table by the name string_vetoed_sngl already exists it is first
...
DROP TABLE IF EXISTS string_vetoed_sngl;
	...
	if temporary:
		cursor.execute("""
CREATE TEMPORARY TABLE string_vetoed_sngl (event_id TEXT PRIMARY KEY)
		""")
	else:
		cursor.execute("""
...
WHERE
	EXISTS (
		...
		FROM
			segment_definer
			JOIN segment ON (
				segment.segment_def_id == segment_definer.segment_def_id
			)
		WHERE
			segment_definer.name == ?
			AND segment_definer.ifos LIKE '%' || sngl_burst.ifo || '%'
			AND sngl_burst.peak_time + 1e-9 * sngl_burst.peak_time_ns BETWEEN segment.start_time + 1e-9 * segment.start_time_ns AND segment.end_time + 1e-9 * segment.end_time_ns
	)
	""", (veto_segments_name,))
	cursor.close()
