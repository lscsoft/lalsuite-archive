...
def makeOutputPage(objs, params, outdir, confidencelevels):
    ...
    GreedyRes={'mc':0.025,'m1':0.1,'m2':0.1,'mass1':0.1,'mass2':0.1,'mtotal':0.1,'eta':0.001,'iota':0.01,'time':1e-4,'distance':1.0,'dist':1.0,'mchirp':0.025,'a1':0.02,'a2':0.02,'phi1':0.05,'phi2':0.05,'theta1':0.05,'theta2':0.05,'ra':0.05,'dec':0.05}
    ...
    std={}
    for par in params:
        if not reduce( lambda a,b: (par in a.names()) and (par in b.names()),objs):
    ...
    for p1 in params: # X axis, injection values
        try:
            injval=map(lambda o: o._getinjpar(p1))
        except KeyError:
    ...
    for p1 in params: # X axis, injection values
        ...
        for p2 in params:
            try:
                ...
                continue
            fig = plt.errorbar(injval,mean[p2],std[p2])
            fig.xlabel(p1)
            fig.ylabel(p2)
            figname=p1+'-'+p2+'.png'
    ...
    # Save page
    resultspage=open(os.path.join(outdir,'injectionsummary.html'))
    resultspage.write(str(html))
    resultspage.close()

def scatterAcc(x,y,xname,yname):
    myfig = plt.scatter(x,y,label='Std Acc of %s'%(yname))
    plt.xlabel(xname)
    plt.ylabel(yname)
    return myfig
