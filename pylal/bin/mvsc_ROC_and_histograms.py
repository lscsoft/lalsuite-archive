...
for i,var in enumerate(variables):
	...
	pylab.title("S6b Chunk 1 CAT 4 triggers",fontsize=18) 
...
for i,var in enumerate(variables):
	...
	try:
		n,bin,patches = pylab.hist(bk_data[var],bins=100,range=(min_x,max_x),normed=True,log=False,label="background")
	except ValueError:
		continue
	pylab.setp(patches,'facecolor','k','alpha',.5)
	pylab.hold(1)
	n,bin,patches = pylab.hist(sg_data[var],bins=100,range=(min_x,max_x),normed=True,log=False,label="signal")
	pylab.setp(patches,'facecolor','r','alpha',.5)
	n,bin,patches = pylab.hist(zl_data[var],bins=100,range=(min_x,max_x),normed=True,log=False,label="zerolag")
	pylab.setp(patches,'facecolor','b','alpha',.5)
...
pylab.figure(1000)
pylab.semilogy(FAP,TAP, linewidth = 2.0)
pylab.hold(True)
