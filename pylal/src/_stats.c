...
static int rankdata(const double *array, const size_t len, double *out_array);
...
static int rankdata(const double *array, const size_t len, double *out_array) {
    ...
    /* want to argsort, so must do some indirection */
    int cmp(const void *a, const void *b) {
        /* cast the pointer then dereference it */
        double c, d;
        c = array[*(const size_t *)a];
        d = array[*(const size_t *)b];
        if (c > d) return 1;
        if (c < d) return -1;
        return 0;
    }

    /* initialize ind_array to indices */
    ind_array = (size_t *)malloc(len * sizeof(size_t));
    if (!ind_array) return 1;
    for (i = 0; i < len; i++) ind_array[i] = i;

    /* argsort */
    qsort(ind_array, len, sizeof(size_t), cmp);
