...
INT4 IMRIstart(REAL8 m, REAL8 M, REAL8 q, REAL8 D, REAL8 Sdotn, REAL8 phi0, REAL8 p0,
	...
	REAL8 hp_interp[20];
	REAL8 hx_interp[20];
	REAL8 dhpdi_interp[20];
	REAL8 dhxdi_interp[20];
	...
	//At each index, get derivative dh/di of each polarization using interpolation functions
	gsl_spline_init(spline_hp, i_interp, hp_interp, 20);
	gsl_spline_init(spline_hx, i_interp, hx_interp, 20);
	for (INT4 i=0; i<20; i++) {
		dhpdi_interp[i] = gsl_spline_eval_deriv(spline_hp, i_interp[i], acc);
		dhxdi_interp[i] = gsl_spline_eval_deriv(spline_hx, i_interp[i], acc);
		}
