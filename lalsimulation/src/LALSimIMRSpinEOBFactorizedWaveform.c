 ...
 * (ii) \f$exp(\mathrm{i} deltalm)\f$ set to 1.
...
)
{
	...
	const	REAL8 vPhiKepler = params->alignedSpins ?
					...
					XLALSimIMRSpinEOBNonKeplerCoeff(cartvalues->data, params);
...
/*
 * The following functions are to be used by the SEOBNRv3 waveform
 * generation. Including a function to be used for flux calculation.
 */
...
)
{
	...
	/*
	 * if (r > 8.5) { printf("YP::dynamics variables in waveform: %i, %i,
	 * %e, %e, %e\n",l,m,r,pp,values->data[1]); printf( "rholm^l = %.16e,
	 * Tlm = %.16e + i %.16e, |Tlm| = %.16e \nSlm = %.16e, hNewton =
	 * %.16e + i %.16e, delta = %.16e\n", rholmPwrl, creal(Tlm),
	 * cimag(Tlm), cabs(Tlm), Slm, creal(hNewton), cimag(hNewton),
	 * deltalm ); printf("delta22 coeffs: vh3C = %.16e, vh6C = %.16e,
	 * vh9C = %.16e, v5C = %.16e, v6C = %.16e, v8C =
	 * %.16e\n",hCoeffs->delta22vh3,hCoeffs->delta22vh6,hCoeffs->delta22vh
	 * 9,hCoeffs->delta22v5,hCoeffs->delta22v6,hCoeffs->delta22v8);
	 * printf("hNewt amp = %.16e, arg =
	...
	*hlm *= hNewton;
	/*
	 * if (r > 8.5) { printf("YP::FullWave: Reh = %.16e, Imh = %.16e,
	 * hAmp = %.16e, hPhi =
