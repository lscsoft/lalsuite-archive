...
#include "LALSimIMRTNSEOBFactorizedWaveform.c"
#include "LALSimIMREOBFactorizedWaveform.c"
#include "LALSimIMRTNSEOBFactorizedFlux.c"
...
static
REAL8 XLALvrP4PN(const REAL8 r, const REAL8 omega, const REAL8 eta, const REAL8 mass1, const REAL8 mass2, const REAL8 lambda1, const REAL8 lambda2);
            ...
            )
{
  ...
  A  = XLALCalculateTNSEOBA( r, m1,m2, lambda1, lambda2 );
  //dA = - r*r * XLALCalculateEOBdAdr( r, eta ); REALLY???!?!?!
  //dA is dAdu NOT dAdr
  dA =  XLALCalculateEOBdAdu(r,m1,m2,lambda1,lambda2);
 // printf("In the function pphiInit");
 // printf("\n%f",dA/(2.*u*A + u2 * dA));
 // printf("dA/(2.*u*A + u2 * dA)....");
  
//  printf("\n%f",sqrt(-dA/(2.*u*A + u2 * dA)));
//  printf("q");
...
REAL8
XLALprInitP4PN(REAL8 p,     /**<< The pr value we are currently testing */
             ...
             )
{
  ...
  REAL8 eta,  mass1, mass2, lambda1, lambda2, z3, r, vr, q;
  TNSPr3In *ak;
             ...
             )
{
  ...
  // coeffs = (EOBACoefficients *) params;
   ...
   A  = XLALCalculateTNSEOBA( r, mass1,mass2, lambda1, lambda2 );
   dA = -u2*  XLALCalculateEOBdAdu(r,mass1,mass2,lambda1,lambda2);
  // printf("\n%f",A);
  // printf("A..");
   /* Comparing this expression with the old one, I *think* I will need to multiply */
   /* dA by - r^2. TODO: Check that this should be the case */
 ...
 //  printf("\n%f",u3);
 //  printf("u");
 //  printf("\n%f",-0.5 * dA /(1. + 2.*eta * (A/sqrt(A+0.5 * u*dA)-1.)));
//   printf("This is  -0.5 * dA /(1. + 2.*eta * (A/sqrt(A+0.5 * u*dA)-1.)))");
//   printf("\n%f",(A+0.5 * u*dA));
//   printf("(A+0.5 * u*dA))");
//   printf("\n%f",omega);
//   printf("omega");
            ...
            )
{
 ...
 TNSPr3In *TNSpr3In;
  ...
  omega2 = omegaofrP4PN(r, TNSpr3In->eta, TNSpr3In->mass1,TNSpr3In->mass2, TNSpr3In->lambda1, TNSpr3In->lambda2);
                      ...
                      )
{
  ...
  REAL8 eta, omega, mass1, mass2, lambda1, lambda2;
...
//Radial flux corrections
REAL8 c1=-(227.*eta/140.)+(1957./1680.);
REAL8  c2=(753.*eta*eta/560.)+(165703.*eta/70560.)-(25672541./5080320.);

//  REAL8 flux_radial=-5./3. * (p/q)* flux *(1.+(c1*u)+(c2*u2));


  A          = XLALCalculateTNSEOBA(r, mass1,mass2,lambda1,lambda2 );                                                                                                                                    
  dAdr       = -u2 *  XLALCalculateEOBdAdu(r,mass1,mass2,lambda1,lambda2);                                                                                                                               
  ...
  Heff  = XLALEffectiveHamiltonian( eta,mass1,mass2, lambda1,lambda2, r, p, q );                                                                                                                         
  ...
  flux = XLALSimIMRTNSEOBFactorizedFlux( &valuesVec, omega, params, lMax,eta,mass1,mass2, lambda1,lambda2 );                                                                                                                              
                                                                                                                                                                                                         
  /* pDot */                                                                                                                                                                                             
  dvalues[2] = (0.5 * AoverSqrtD * u3 * ((  2.0 * ( q2 + p4 * z3) * A                                                                                                                                      
                     - r * ( q2 + r2 + p4 * z3 ) * dAdr )) / HeffHreal)                                                                                                                                    
                     + ((5.*p) /(3.* q)) * (flux *(1.+(c1*u)+(c2*u2))/ (eta * omega));                                                                                                                                                 
                                                                                                                                                                                                         
  /* qDot */                                                                                                                                                                                             
  dvalues[3] = - flux / (eta * omega);                                                                                                                                                                   

   // printf("THE RHS : \n%f\t%f\t%f\t%f", (double)dvalues[0], (double) dvalues[1], (double) dvalues[2], (double) dvalues[3] );
   // printf("\n THE Conservative quantity 1) Heff 2) Hreal 3)HeffHreal 4)A 5)D : \n%f\t%f\t%f\t%f\t%f", (double)Heff, (double) Hreal, (double) HeffHreal, (double) A,(double)XLALCalculateEOBD(r,eta) );
                        ...
                        )
{
  ...
  A    = XLALCalculateTNSEOBA( r, mass1,mass2, lambda1, lambda2 );
  Heff = XLALEffectiveHamiltonian( eta,mass1,mass2,lambda1,lambda2, r, pr, pPhi );
  Hreal = sqrt( 1. + 2.*eta*(Heff - 1.) );
...
/**
 * Calculates the initial radial velocity CHANGE THIS!!!!!!!!!!!!!!!!!!!!!!!!!!!!
 */
                 ...
                 const REAL8 lambda2)
{
  ...
  dA  = XLALCalculateEOBdAdu(r,mass1,mass2,lambda1,lambda2);
  d2A =  XLALCalculateEOBd2Adu2(r,eta,mass1,mass2,lambda1,lambda2);
              ...
              )
{
  ...
  // EOBACoefficients        aCoeffs;
   ...
   /* THIS IS ONLY LET U PRODUCE (2,2).... none of the others are actually coded up for TNSEOB yet */
   ...
   if ( !values || !dvalues )
   {
    ...
    printf("entered generator function:");
   ...
   eobParams.eta = eta;
   eobParams.mass1  = mass1;
   eobParams.mass2  = mass2;
   eobParams.lambda1 = lambda1;
   eobParams.lambda2 = lambda2;
...
printf("Getting FactWaveformCoeff:");
printf("eta that goes into making hCoeff : %g\n", (double) eta);


   if ( XLALSimIMRTNSEOBCalcFacWaveformCoefficients( &hCoeffs, eta) == XLAL_FAILURE 
   ||   XLALSimIMREOBComputeNewtonMultipolePrefixes( &prefixes, eobParams.mass1, eobParams.mass2 )
         == XLAL_FAILURE )
   {
     XLALDestroyCOMPLEX16Vector( modefreqs );
     XLALDestroyREAL8Vector( values );
  ...
  // TNSpr3In.aCoeffs = &aCoeffs;
   ...
   TNSpr3In.mass1=mass1;
   TNSpr3In.mass2=mass2;
   TNSpr3In.lambda1=lambda1;
   TNSpr3In.lambda2=lambda2;
   ...
   /* to start at least at r = rmin (in units of M). */
    ...
    // TODO: CONVINCE PRAYUSH THAT THE NEXT 5 LINES ARE USEFUL
   r = (r<rmin) ? rmin : r;
   TNSpr3In.r = r;
// printf("Calculating omega");
   ...
   omega = omegaofrP4PN( r, eta, mass1,mass2,lambda1,lambda2 );
   TNSpr3In.omega = omega;
 //  printf("\n%f",omega);
 //  printf("omega");
   
   q = XLALpphiInitP4PN(r, mass1, mass2, lambda1, lambda2 );
  // printf("\n%f",q);
  // printf("q");   
/* first we compute vr (we need coeef->Fp6) */
   TNSpr3In.q = q;
   TNSpr3In.vr = XLALvrP4PN(r, omega, eta, mass1,mass2,lambda1,lambda2);
   ...
   p = p * XLALCalculateTNSEOBA(r, mass1,mass2, lambda1, lambda2);
...
// TODO: PUT HERE THE ARTIFICIAL ASSIGNMENT OF VALUES->DATA
    // TODO: GO INSIDE LALHCapDerivativesP4PN, AND ADD PRINT STATEMENTS FOR R, V, OMEGA, H, F_LM, H_LM, S_LM, T_LM, EPS_LM, HNEWTONIAN_LM
    
    
   // values->data[0] = 30.0;
   // values->data[1] = 0.0;
   // values->data[2] =  -0.0001;
   //  values->data[3] = 5.0;
    
    
    
    // DELME: ULTIMATE, WEL'LL BE READING IN A FILE HERE. THE FILE WILL HAVE THE TRAJECTORY'S COORDINATES (R,PHI,PR,PPHI). AND WE;LL BE CALCULATING HTE RHS OF THE EOM AND STORING THEM IN A FILE HERE.
...
printf("Entering the intergator:");
...
/* Set up the v:ectors, and re-initialize everything for the high sample rate */
...
printf("Initial cond for the high resolution:");
printf("r %g\n", (double) rVec.data[hiSRndx]);



FILE *fp1;

fp1=fopen("Dynamical_variables_in_low_Res.txt", "w");
if(fp1 == NULL)
    exit(-1);
	int cellno;
	for ( cellno=0; cellno<retLen; cellno=cellno+1 )
	{
	   fprintf(fp1,"\n%f\t%f\t%f\t%f\t%f", (double) dynamics->data[cellno], (double) rVec.data[cellno], (double) phiVec.data[cellno], (double) pPhiVec.data[cellno], (double) prVec.data[cellno]);
	   }
...
fp2=fopen("Dynamical_variables_in_high_Res.txt", "w");
if(fp2 == NULL)
    exit(-1);
		    for ( cellno=0; cellno<retLen; cellno=cellno+1 )
			    {
				       fprintf(fp1,"\n%f\t%f\t%f\t%f\t%f", (double) dynamicsHi->data[cellno], (double) rVecHi.data[cellno], (double) phiVecHi.data[cellno], (double) pPhiVecHi.data[cellno], (double) prVecHi.data[cellno]);
					          }
...
printf("We are now calling fucntion to debug the flux (Jan 23)");

//Pick the dynamical Variables and omega! :   
REAL8 values1[4];
values1[0]=   5.5598086901;  //r
values1[1]=   5885.2197561442;   //phi
values1[2]=    -0.0381846639;  //pr
values1[3]=   3.2441094290;  //pphi
omega=  0.0739625588;

REAL8Vector values1Vec;
values1Vec.length = 4;
values1Vec.data=values1;


REAL8 flux_test = XLALSimIMRTNSEOBFactorizedFlux_Debugging( &values1Vec, omega, &eobParams, 8,eta,mass1,mass2, lambda1,lambda2 );

//At this point I get F_lm given in equation 40

REAL8 flux_full=- flux_test/(eta * omega);

printf("F_phi* that goes into EOM: %g\n", (double) flux_full); 
  ...
  while ( i < hiSRndx )
  {
    omega = XLALCalculateOmega( eta,mass1,  mass2, lambda1, lambda2, rVec.data[i], prVec.data[i], pPhiVec.data[i] );
  ...
  {
    omega = XLALCalculateOmega( eta,mass1,mass2,lambda1,lambda2, rVecHi.data[i], prVecHi.data[i], pPhiVecHi.data[i] );
  ...
  for ( currentMode = 0; currentMode < nModes; currentMode++ )
  {
    ...
    for ( i=0; i < (UINT4)retLen; i++ )
    {
      ...
      xlalStatus = XLALSimIMRTNSEOBGetFactorizedWaveform( &hLM, values, v, modeL, modeM, &eobParams );
...
{ 
   printf("Entered Dominant:");;
  if ( XLALSimIMRTNSEOBGenerator(hplus, hcross, NULL, phiC, deltaT, m1SI, m2SI,
              fLower, distance, inclination,0, lambda1, lambda2 ) == XLAL_FAILURE )
  {printf("Error in Generator");

    XLAL_ERROR( XLAL_EFUNC );
  }
printf("generator success");
  return XLAL_SUCCESS;
}
