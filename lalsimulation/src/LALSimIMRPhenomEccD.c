...
#include <lal/Units.h>
*/
#include "LALSimIMRPhenomEccD_internals.c"
UsefulPowers POWERS_of_pi;	// declared in LALSimIMRPhenomEccD_internals.c
				// renamed to avoid conflict with IMRPhenomD
...
static int IMRPhenomEccDGenerateFD(
 ...
 *
 * ADDED: Leading-order eccentricity corrections up to 3PN order in inspiral phase
 * (Details: see LALSimInspiralEccTF2.c )
 */
int XLALSimIMRPhenomEccDGenerateFD(
    COMPLEX16FrequencySeries **htilde, /**< FD waveform */
  ...
  if (e_min < 0) XLAL_ERROR(XLAL_EDOM, "e_min must be positive\n");
  if (e_min >= 1.) XLAL_ERROR(XLAL_EDOM, "e_min must be less than 1\n");
  ...
  int status = IMRPhenomEccDGenerateFD(htilde, phi0, fRef, deltaF,
                                    m1, m2, chi1, chi2,
                                    f_min, f_max_prime, distance, e_min);
  XLAL_CHECK(XLAL_SUCCESS == status, status, "Failed to generate IMRPhenomEccD waveform.");
...
/* *********************************************************************************/
/* The following private function generates IMRPhenomEccD frequency-domain waveforms  */
/* given coefficients */
/* *********************************************************************************/
...
static int IMRPhenomEccDGenerateFD(
  ...
  *htilde = XLALCreateCOMPLEX16FrequencySeries("htilde: FD waveform", &ligotimegps_zero, 0.0,
  ...
  XLALSimInspiralEccTF2AlignedPhasing(&pn, &pfv19by3, &pfv25by3, &pfv28by3, &pfv31by3, &pfv34by3, &pfv37by3, m1, m2, chi1, chi2, 1.0, 1.0, e_min, f_min, LAL_SIM_INSPIRAL_SPIN_ORDER_35PN);
  ...
  ComputeIMRPhenEccDPhaseConnectionCoefficients(pPhi, pn, &phi_prefactors, pfv19by3, pfv25by3, pfv28by3, pfv31by3, pfv34by3, pfv37by3);
  ...
  for (size_t i = ind_min; i < ind_max; i++)
  {
	...
	else
	{
		 ...
		 REAL8 phi = IMRPhenEccDPhase(Mf, pPhi, pn, &powers_of_f, &phi_prefactors, pfv19by3, pfv25by3, pfv28by3, pfv31by3, pfv34by3, pfv37by3);
		 ...
		 {
			 REAL8 phifRef = IMRPhenEccDPhase(MfRef, pPhi, pn, &powers_of_fRef, &phi_prefactors, pfv19by3, pfv25by3, pfv28by3, pfv31by3, pfv34by3, pfv37by3);
