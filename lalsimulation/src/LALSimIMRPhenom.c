...
/********************************************************************************/
/********************************************************************************/
/* structure containing the coefficients of the series expansion of the derivatives */ 
typedef struct tagIMRDerivCoeffs{
  REAL8 dA1dM_0;
  REAL8 dA1dM_1;
  REAL8 dA1dM_2;

  // and so on... complete this 
...
IMRDerivCoeffs *XLALComputeIMRPhenomBDerivativeCoeffs(
	...
	params = XLALComputeIMRPhenomBParams(m1, m2, chi);
	...
	derCoeffs->dA1dM_1 = 0.;
	derCoeffs->dA1dM_2 = 0.6846531968814576*alpha2*sqrt(eta*M);
	
	//...  and so on.. get the optimized version of the code generated by Mathematica. FIXME 
...
gsl_matrix *XLALSimIMRPhenomBFisherMatrix(
	...
	/* initialize the elements of the Fisher matrix */
	REAL8 gamma_MM = 0.;
	REAL8 gamma_MEta = 0.;
	// .. and so on.. FIXME 
	...
	*  and use this to compute the Fisher matrix */
    for (;k--;) {
		...
		const REAL8 f_pow_m7by6 = pow(f, -7./6); 
		const REAL8 Shf = Sh->data[k];
		// ... and so on 

		/* compute derivatives of the amplitude w.r.t M, eta and chi */
		if (f <= fMerg) {
			dAdM = dA1dM_0*f_pow_m7by6 + dA1dM_1*f_pow_m5by6 + dA1dM_2*f_pow_m3by6 + dA1dM_3*f_pow_m1by6;
			dAdEta = dA1deta_0*f_pow_m7by6 + dA1deta_1*f_pow_m5by6 + dA1deta_2*f_pow_m3by6 + dA1deta_3*f_pow_m1by6;
			dAdChi = dA1dchi_0*f_pow_m7by6 + dA1dchi_1*f_pow_m5by6 + dA1dchi_2*f_pow_m3by6 + dA1dchi_3*f_pow_m1by6;
		}
		else if (f > fRing) {
			dAdM = dA2dM_0*f_pow_m2by3 + dA2dM_1*f_pow_m1by3 + dA2dM_2;
			dAdEta = dA2deta_0*f_pow_m2by3 + dA2deta_1*f_pow_m1by3 + dA2deta_2;
			dAdChi = dA2dchi_0*f_pow_m2by3 + dA2dchi_1*f_pow_m1by3 + dA2dchi_2;
		}
		else {
			// ... fill in the derivative of the ringdown amplitude FIXME 
		}

		/* compute derivatives of the phase w.r.t M, eta and chi */
		dPsidM =  // ... fill in the phase derivative FIXME 
		dPsidEta = 
		dPsidChi = 

		/* compute the Fisher matrix */ 
		gamma_MM += (Asqr*dPsidM*dPsidM + dAdM*dAdM)/Shf; 
		gamma_MEta += (Asqr*dPsidM*dPsidEta + dAdM*dAdEta)/Shf; 
		gamma_MChi += (Asqr*dPsidM*dPsidChi + dAdM*dAdChi)/Shf; 
		gamma_etaEta += (Asqr*dPsidEta*dPsidEta + dAdEta*dAdEta)/Shf; 
		gamma_etaChi += (Asqr*dPsidEta*dPsidChi + dAdEta*dAdChi)/Shf; 
		gamma_chiChi += (Asqr*dPsidChi*dPsidChi + dAdChi*dAdChi)/Shf; 
		// .. fill in the t0, phi0 terms.. FIXME 
	}

	/* normalization of the Fisher matrix by the templ norm sqr */ 
	gamma_MM *= 0.5*df/hSqr; 
	gamma_MEta *= 0.5*df/hSqr; 
	// ... and so on. FIXME 

	/* fill the gsl matrix containing the Fisher matrix in (M, eta, chi, t0, phi0) */ 
	gsl_matrix * g = gsl_matrix_calloc (5, 5);
	
	gsl_matrix_set (g, 0,0, gamma_MM); 
	gsl_matrix_set (g, 0,1, gamma_MEta); 
	// ... and so on. FIXME 
...
int XLALSimIMRPhenomBMetricInMEtaChi(
	REAL8 *gamma00,  	/**< template metric coeff. 00 */
	REAL8 *gamma01,  	/**< template metric coeff. 01 */
	REAL8 *gamma11,  	/**< template metric coeff. 11 */
    const REAL8 m1,     /**< component mass 1 (kg) */ 
    const REAL8 m2,     /**< component mass 2 (kg) */ 
    const REAL8 chi,    /**< effective spin parameter of IMRPhenomB: chi = (m1 chi1 + m2 chi2)/(m1+m2)  */ 
    const REAL8 fLow,   /**< low-frequency cutoff (Hz) */
	...
	gsl_matrix * g = XLALSimIMRPhenomBFisherMatrix(m1, m2, chi, fLow, df, Sh); 

	/* project out dimensions t0 and phi0 */ 
	// complete this ... FIXME 
...
/**
 * Compute the template-space metric of the IMRPhenomB waveform in the 
 * (xi0, xi3, xi3s) coordinates
 */
gsl_matrix *XLALSimIMRPhenomBMetric(
	REAL8 *gamma00,  	/**< template metric coeff. 00 */
	REAL8 *gamma01,  	/**< template metric coeff. 01 */
	REAL8 *gamma11,  	/**< template metric coeff. 11 */
    const REAL8 m1,     /**< component mass 1 (kg) */ 
	...
	/* compute the metric in the (M, eta, chi) coordinates */
	status = XLALSimIMRPhenomBMetricInMEtaChi(gamma00, gamma01, gamma11, m1, m2, chi, fLow, df, Sh);

	/* make sure that the metric in M, eta, chi was computed successfully */ 
	if (status == XLAL_SUCCESS) {
	
		/* transform the metric into (xi0, xi3, xi3s) coordinates using the appropriate Jacobian */
