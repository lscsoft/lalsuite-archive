  ...
  const LALSimInspiralTestGRParam *extraParams) /**<linked list containing the extra testing GR parameters */
{
  ...
  int retcode = PhenomPCore(hptilde, hctilde,chi1_l, chi2_l, chip, thetaJ, m1_SI, m2_SI, distance, alpha0, phic, f_ref, freqs, deltaF, IMRPhenomP_version, extraParams);
...

{
  ...
  int retcode = PhenomPCore(hptilde, hctilde,chi1_l, chi2_l, chip, thetaJ, m1_SI, m2_SI, distance, alpha0, phic, f_ref, freqs, 0, IMRPhenomP_version, extraParams);
  XLAL_CHECK(retcode == XLAL_SUCCESS, XLAL_EFUNC, "Failed to generate IMRPhenomP waveform.");
