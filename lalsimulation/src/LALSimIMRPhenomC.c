...
int XLALSimIMRPhenomCGenerateTD(
	...
	printf("In XLALSimIMRPhenomCGenerateTD, fRingDown = %f fCut = %f\n",
            params->fRingDown, params->fCut);
	fflush(NULL);
	#endif
...
static int IMRPhenomCGenerateTD(
    ...
    *ind_t0 = ((nt - EstimateIMRLength(m1, m2, f_min_wide, deltaT)) > (4 * ne)) ? (nt -(2* ne)) : (nt - (1 * ne));
	...
	#if debugPrayush
	printf("In IMRPhenomCGenerateTD: t0 passed = %f, fRD = %f Hz\n", t0,
            params->fRingDown);
	#endif
...
static size_t find_instant_freq(const REAL8TimeSeries *hp, const REAL8TimeSeries *hc, const REAL8 target, const size_t start) {
	...
	printf("In find_instant_freq: target_ind = %zu\n", target_ind);
	fflush(NULL);
	#endif
