...
static double
GSLSpinHamiltonianDerivWrapperPrec_exact(double x,	/**<< Derivative at x */
 ...
 * This static function is called only by XLALSimIMRSpinEOBInitialConditionsPrec_exact
 */
static int
 ...
 * GSLSpinHamiltonianDerivWrapperPrec_exact and XLALSimIMRSpinEOBInitialConditionsPrec_exact
 */
static int
 ...
 * In the code from Tyson Littenberg, this was only applicable to the special theta=pi/2, phi=0 case.
 * This special transformation is a static function called only by XLALSimIMRSpinEOBInitialConditionsPrec_exact
 */
static int
 ...
 */
static int
XLALFindSphericalOrbitPrec_exact(
...
)
{
	...
	if (debugPK)
		XLAL_PRINT_INFO("\n\nmtotal in XLALFindSphericalOrbitPrec_exact = %f\n", (float)mTotal);
...
static REAL8
XLALCalculateSphHamiltonianDeriv2Prec_exact(
...
)
{
	 ...
	 */
	F.function = GSLSpinHamiltonianDerivWrapperPrec_exact;
	F.params = &dParams;
...
static int
XLALSimIMRSpinEOBInitialConditionsPrec_exact(
...
)
{
  ...
  FILE* UNUSED out = NULL;
	...
	if (printPK) {
		XLAL_PRINT_INFO("Inside the XLALSimIMRSpinEOBInitialConditionsPrec_exact function!\n");
		XLAL_PRINT_INFO(
  ...
  /* Initialise the gsl stuff */
	...
	rootFunction.f = XLALFindSphericalOrbitPrec_exact;
	rootFunction.n = 3;
...
//          gsl_vector_get(finalValues, 2)/scale3);
  ...
  if(debugPK) { fflush(NULL); fclose(out); }
	...
	d2Hdr2 = XLALCalculateSphHamiltonianDeriv2Prec_exact(0, 0, sphValues, params);
	d2Hdrdpphi = XLALCalculateSphHamiltonianDeriv2Prec_exact(0, 5, sphValues, params);
	...
	//XLALSpinPrecHcapExactDerivWRTParam(4, cartValues, params) / sphValues[0];
	...
	if (d2Hdr2 != 0.0) {
		...
		//XLALSpinPrecHcapExactDerivWRTParam(3, cartValues, params);
			       ...
			       void *params /**<< Function parameters */ )
{
	...
	if (debugPK) {
		XLAL_PRINT_INFO("In GSLSpinHamiltonianDerivWrapperPrec_exact");
		XLAL_PRINT_INFO("rvec = %.16e %.16e %.16e\n", rvec[0], rvec[1], rvec[2]);
		XLAL_PRINT_INFO("pvec = %.16e %.16e %.16e\n", pvec[0], pvec[1], pvec[2]);
		XLAL_PRINT_INFO("theta1 = %.16e\n", theta1);
		XLAL_PRINT_INFO("theta2 = %.16e\n", theta2);
	...
	if (theta1 > 1.0e-6 && theta2 >= 1.0e-6) {
		...
		case 0:
			/* dHdr */
			dHdx = -tmpDValues[3];
			//XLALSpinPrecHcapExactDerivWRTParam(0, cartValues, dParams->params);
			dHdpy = tmpDValues[1];
			//XLALSpinPrecHcapExactDerivWRTParam(4, cartValues, dParams->params);
			dHdpz = tmpDValues[2];
			//XLALSpinPrecHcapExactDerivWRTParam(5, cartValues, dParams->params);
...
#endif // _LALSIMIMRSPINPRECEOBINITIALCONDITIONS_V3OPT_C
