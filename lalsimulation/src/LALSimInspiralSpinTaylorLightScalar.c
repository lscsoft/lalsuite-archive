...
typedef struct tagXLALSimInspiralSpinTaylorTxCoeffs
{
  ...
  REAL8 nu;
  REAL8 mScalar;
  REAL8 wdotanomalous1;
  REAL8 wdotanomalous2;
	...
	REAL8 fCONT; ///< contact frequency for the case of BNS or NSBH.
...
static int XLALSimInspiralSpinTaylorT4LightScalarStoppingTest(double t,
	const double values[], double dvalues[], void *mparams);
static int XLALSimInspiralSpinTaylorT4LightScalarDerivatives(double t,
	const double values[], double dvalues[], void *mparams);
static int XLALSimInspiralSpinTaylorT4SetupLightScalar(
    XLALSimInspiralSpinTaylorTxCoeffs *params, REAL8 m1, REAL8 m2,
    REAL8 fStart, REAL8 fEnd, REAL8 lambda1, REAL8 lambda2,
    REAL8 sens1, REAL8 sens2, REAL8 omegaBD, REAL8 mScalar,
    REAL8 quadparam1, REAL8 quadparam2, LALSimInspiralSpinOrder spinO,
    LALSimInspiralTidalOrder tideO, INT4 phaseO, LALSimInspiralTestGRParam *extraParams);
static int XLALSimInspiralSpinTaylorT4LightScalarDriver(REAL8TimeSeries **hplus,
    ...
    LALEquationOfState eos_type, REAL8 omegaBD, REAL8 mScalar, REAL8 lambda1, REAL8 lambda2, REAL8 quadparam1, REAL8 quadparam2,
    LALSimInspiralSpinOrder spinO, LALSimInspiralTidalOrder tideO,
    int phaseO, int amplitudeO, Approximant approx, LALSimInspiralTestGRParam *extraParams);
static int XLALSimInspiralSpinTaylorLightScalarPNEvolveOrbitIrregularIntervals(
    ...
    REAL8 lnhaty, REAL8 lnhatz, REAL8 e1x, REAL8 e1y, REAL8 e1z,
    LALEquationOfState eos_type, REAL8 omegaBD, REAL8 mScalar,
    ...
    Approximant approx, LALSimInspiralTestGRParam *extraParams);
static int XLALSimInspiralSpinTaylorLightScalarDriverFourier(
    ...
    REAL8 lambda1, REAL8 lambda2, REAL8 quadparam1, REAL8 quadparam2,
    LALSimInspiralSpinOrder spinO, LALSimInspiralTidalOrder tideO, INT4 phaseO,
    INT4 amplitudeO, Approximant approx, INT4 phiRefAtEnd, LALSimInspiralTestGRParam *extraParams);


REAL8 StepFunction(REAL8 f, REAL8 x);
REAL8 StepFunction(REAL8 f, REAL8 x){
  REAL8 a = 0.0 ;
  if (f >= x) a = 1.0 ;
  return a ;
...
static int XLALSimInspiralSpinTaylorT4SetupLightScalar(
  ...
  REAL8 sens1,                    /**< neutron star "sensitivity" */
  REAL8 sens2,                    /**< neutron star "sensitivity" */
  REAL8 omegaBD,                  /**< Brans-Dicke parameter */
  REAL8 mScalar,                  /**< Mass of the light scalar in eV  */
...
{
    REAL8 M, eta, Mchirp, dm, m1m2, m2m1, m1M, m2M, ksi, Gamma, curlyS;
    REAL8 hbar = 6.582e-16; /* hbar in eV.s */
    ...
    ksi = 1.0/(2.0 + omegaBD);
    Gamma = 1.0 - 2.0*(sens1*m2 + sens2*m1)/M;
    curlyS = sens2 - sens1;
    params->mScalar = mScalar/hbar; /* params->mScalar is in Hz */
    params->nu = params->mScalar*params->mScalar*M*M;
    fprintf(stdout,"nu      = %.4e\n",params->nu) ;
    params->wdotanomalous1 = 5.0*ksi*curlyS*curlyS/48.0;
    params->wdotanomalous2 = Gamma*Gamma*ksi/12.0;
    ...
    switch( phaseO )
    {
        ...
        case 0:
            params->wdotcoeff[0] = 1. - ksi/2.0 - 2.0*ksi*(sens1 + sens2 - 2.0*sens1*sens2)/3.0 ;
            params->Ecoeff[0] = 1.;
            break;
        default:
    ...
    {
      REAL8 dchi=0.0;
        if (XLALSimInspiralTestGRParamExists(extraParams,"dchi0"))  params->wdotcoeff[0]*=1.0+XLALSimInspiralGetTestGRParam(extraParams,"dchi0");
        if (XLALSimInspiralTestGRParamExists(extraParams,"dchi1"))  params->wdotcoeff[1]=XLALSimInspiralGetTestGRParam(extraParams,"dchi1");
        if (XLALSimInspiralTestGRParamExists(extraParams,"dchi2"))  params->wdotcoeff[2]*=1.0+XLALSimInspiralGetTestGRParam(extraParams,"dchi2");
        if (XLALSimInspiralTestGRParamExists(extraParams,"dchi3"))  {
	  dchi = XLALSimInspiralGetTestGRParam(extraParams,"dchi3");
	  params->wdotcoeff[3] *= 1.0 + dchi;
	  params->wdotSO15s1 *= 1.0 + dchi;
	  params->wdotSO15s2 *= 1.0 + dchi;
	}
        if (XLALSimInspiralTestGRParamExists(extraParams,"dchi4"))  {
	  dchi = XLALSimInspiralGetTestGRParam(extraParams,"dchi4");
	  params->wdotcoeff[4] *= 1.0 + dchi;
	  params->wdotSS2 *= 1.0 + dchi;
	  params->wdotQM2S1 *= 1.0 + dchi;
	  params->wdotQM2S2 *= 1.0 + dchi;
	  params->wdotQM2S1L *= 1.0 + dchi;
	  params->wdotQM2S2L *= 1.0 + dchi;
	  params->wdotSSselfS1 *= 1.0 + dchi;
	  params->wdotSSselfS2 *= 1.0 + dchi;
	  params->wdotSSselfS1L *= 1.0 + dchi;
	  params->wdotSSselfS2L *= 1.0 + dchi;
	}
        if (XLALSimInspiralTestGRParamExists(extraParams,"dchi5"))  {
	  dchi = XLALSimInspiralGetTestGRParam(extraParams,"dchi5");
	  params->wdotcoeff[5]*=1.0 + dchi;
	  params->wdotSO25s1 *= 1.0 + dchi;
	  params->wdotSO25s2 *= 1.0 + dchi;
	}
	if (XLALSimInspiralTestGRParamExists(extraParams,"dchi6"))  {
	  dchi = XLALSimInspiralGetTestGRParam(extraParams,"dchi6");
	  params->wdotcoeff[6]*=1.0 + dchi;
	  params->wdotSO3s1 *= 1.0 + dchi;
	  params->wdotSO3s2 *= 1.0 + dchi;
	}
        if (XLALSimInspiralTestGRParamExists(extraParams,"dchi6l")) params->wdotlogcoeff*=1.0+XLALSimInspiralGetTestGRParam(extraParams,"dchi6l");
        if (XLALSimInspiralTestGRParamExists(extraParams,"dchi7"))  {
	  dchi = XLALSimInspiralGetTestGRParam(extraParams,"dchi7");
	  params->wdotcoeff[7]*=1.0 + dchi;
	  params->wdotSO35s1 *= 1.0 + dchi;
	  params->wdotSO35s2 *= 1.0 + dchi;
	}
    }
 ...
 */
int XLALSimInspiralSpinTaylorT4LightScalarPNEvolveOrbit(
  ...
  LALEquationOfState eos_type,    /**< type of EOS */
  REAL8 omegaBD,    		/**< Brans-Dicke parameter */
  REAL8 mScalar,			/**< Scalar mass in eV */
	REAL8 lambda1,                  /**< (tidal deformability of mass 1) / (mass of body 1)^5 (dimensionless) */
	REAL8 lambda2,                  /**< (tidal deformability of mass 2) / (mass of body 2)^5 (dimensionless) */
	REAL8 quadparam1,               /**< phenom. parameter describing induced quad. moment of body 1 (=1 for BHs, ~2-12 for NSs) */
	REAL8 quadparam2,               /**< phenom. parameter describing induced quad. moment of body 2 (=1 for BHs, ~2-12 for NSs) */
	...
	)
{
    ...
    REAL8 sens1 = XLALSimInspiralNSsensitivity(eos_type, m1/LAL_MSUN_SI);
    REAL8 sens2 = XLALSimInspiralNSsensitivity(eos_type, m2/LAL_MSUN_SI);
    ...
    if( approx == SpinTaylorT4LightScalar )
    {
        XLALSimInspiralSpinTaylorTxCoeffs paramsT4;
        XLALSimInspiralSpinTaylorT4SetupLightScalar(&paramsT4, m1, m2, fStart, fEnd,
                lambda1, lambda2, sens1, sens2, omegaBD, mScalar, quadparam1, quadparam2, spinO, tideO, phaseO, extraParams);
        params = (void *) &paramsT4;
    }
    else
    {
        XLALPrintError("XLAL Error - %s: Approximant must be one of SpinTaylorT1Test, SpinTaylorT2Test, SpinTaylorT4Test, SpinTaylorT4LightScalar, but %i provided\n",
                __func__, approx);
        XLAL_ERROR(XLAL_EINVAL);
    ...
    fprintf(stdout,"sens1   = %.3e\n",sens1) ;
    fprintf(stdout,"sens2   = %.3e\n",sens2) ;
    ...
    if( approx == SpinTaylorT4LightScalar )
        integrator = XLALAdaptiveRungeKutta4Init(LAL_NUM_ST4_VARIABLES,
                XLALSimInspiralSpinTaylorT4LightScalarDerivatives,
                XLALSimInspiralSpinTaylorT4LightScalarStoppingTest,
 ...
 * 6) The orbital frequency is >= contact frequency
 * SpinTaylorT4 and SpinTaylorT2 both use this same stopping test
 */
static int XLALSimInspiralSpinTaylorT4LightScalarStoppingTest(
	double t,
	const double values[],
	double dvalues[],
	void *mparams
...
{
    REAL8 omega, v, test, omegaStart, omegaEnd, ddomega, vContact;
    ...
    vContact = pow(LAL_PI * params->M * params->fCONT, 1./3.) ;
    ...
    else if (v >= vContact) /* Stop at contact */
 ...
 */
static int XLALSimInspiralSpinTaylorT4LightScalarDerivatives(
	double t,
	const double values[],
	double dvalues[],
	void *mparams
	)
{
    ...
    /* auxiliary variables */
    REAL8 v, v2, v3, v4, v5, v6, v7, v11, omega2, omega2by2;
    ...
    REAL8 anomalous1 = 0.0, anomalous2 = 0.0 ;

    /* Protect against sqrt(negative numbers)*/
    if (StepFunction(v3/params->M, params->mScalar) != 0.0){
      anomalous1 = params->wdotanomalous1*pow(1.0 - params->nu/v6, 1.5)/v2 ;
    }
    if (StepFunction(2.0*v3/params->M, params->mScalar) != 0.0){
      anomalous2 = params->wdotanomalous2*pow(1.0 - 0.25*params->nu/v6, 2.5) ;
    }

    domega  = params->wdotnewt * v11 * (
            anomalous1 + anomalous2
            + params->wdotcoeff[0]
 ...
 */
int XLALSimInspiralSpinTaylorT4LightScalar(
  ...
  )
{
    ...
    int n = XLALSimInspiralSpinTaylorT4LightScalarDriver(hplus, hcross, phiRef, v0, deltaT,
            ...
            quadparam1, quadparam2, spinO, tideO, phaseO, amplitudeO, approx, extraParams);

    return n;
}

/**
 * Internal driver function to generate any of SpinTaylorT1/T2/T4Test/T4LightScalar
 */
static int XLALSimInspiralSpinTaylorT4LightScalarDriver(
  REAL8TimeSeries **hplus,        /**< +-polarization waveform */
  REAL8TimeSeries **hcross,       /**< x-polarization waveform */
  REAL8 phiRef,                   /**< orbital phase at reference pt. */
  REAL8 v0,                       /**< tail gauge term (default = 1) */
  ...
  )
{
    ...
    if( fRef < LAL_REAL4_EPS )
    {
        ...
        n = XLALSimInspiralSpinTaylorT4LightScalarPNEvolveOrbit(&V, &Phi,
                &S1x, &S1y, &S1z, &S2x, &S2y, &S2z,
                &LNhatx, &LNhaty, &LNhatz, &E1x, &E1y, &E1z,
                deltaT, m1, m2, fS, fE, s1x, s1y, s1z, s2x, s2y, s2z,
                lnhatx, lnhaty, lnhatz, e1x, e1y, e1z, eos_type, omegaBD, mScalar, lambda1, lambda2,
                quadparam1, quadparam2, spinO, tideO, phaseO, approx, extraParams);
        if( n < 0 )
            XLAL_ERROR(XLAL_EFUNC);
    ...
    else /* Start in middle, integrate backward and forward, stitch together */
    {
        ...
        fS = fRef;
        fE = fStart;
        n = XLALSimInspiralSpinTaylorT4LightScalarPNEvolveOrbit(&V1, &Phi1,
                &S1x1, &S1y1, &S1z1, &S2x1, &S2y1, &S2z1,
                &LNhatx1, &LNhaty1, &LNhatz1, &E1x1, &E1y1, &E1z1,
                deltaT, m1, m2, fS, fE, s1x, s1y, s1z, s2x, s2y,
                s2z, lnhatx, lnhaty, lnhatz, e1x, e1y, e1z, eos_type, omegaBD, mScalar, lambda1, lambda2,
                quadparam1, quadparam2, spinO, tideO, phaseO, approx, extraParams);
        ...
        fS = fRef;
        fE = 0.;
        n = XLALSimInspiralSpinTaylorT4LightScalarPNEvolveOrbit(&V2, &Phi2,
...
static int XLALSimInspiralSpinTaylorLightScalarPNEvolveOrbitIrregularIntervals(
        ...
        INT4 phaseO,                    /**< twice post-Newtonian order */
        Approximant approx,              /**< PN approximant (SpinTaylorT1/T2/T4) */
        LALSimInspiralTestGRParam *extraParams
        )
{
    INT4 intreturn;
    void * params;
        ...
        )
{
    ...
    if( fRef < LAL_REAL4_EPS)
    {
        ...
        fE = 0.;
        /* Evolve the dynamical variables */
        n = XLALSimInspiralSpinTaylorLightScalarPNEvolveOrbitIrregularIntervals(&y,
                m1, m2, fS, fE, s1x, s1y, s1z, s2x, s2y, s2z,
                lnhatx, lnhaty, lnhatz, e1x, e1y, e1z,
                eos_type, omegaBD, mScalar,
                lambda1, lambda2, quadparam1, quadparam2, spinO, tideO, phaseO, approx, extraParams);
        if( n < 0 )
            XLAL_ERROR(XLAL_EFUNC);
    ...
    else /* Start in middle, integrate backward and forward, stitch together */
    {
        ...
        fS = fRef;
        fE = fStart;
        n = XLALSimInspiralSpinTaylorLightScalarPNEvolveOrbitIrregularIntervals(&yStart,
        ...
        fS = fRef;
        fE = 0.;
        n = XLALSimInspiralSpinTaylorLightScalarPNEvolveOrbitIrregularIntervals(&yEnd,
                m1, m2, fS, fE, s1x, s1y, s1z, s2x, s2y,
                s2z, lnhatx, lnhaty, lnhatz, e1x, e1y, e1z,
                eos_type, omegaBD, mScalar,
                lambda1, lambda2, quadparam1, quadparam2, spinO, tideO, phaseO, approx, extraParams);
 ...
 *
 */
int XLALSimInspiralSpinTaylorT4LightScalarFourier(
        COMPLEX16FrequencySeries **hplus,        /**< +-polarization waveform */
        COMPLEX16FrequencySeries **hcross,       /**< x-polarization waveform */
        REAL8 fMin,                     /**< minimum frequency of the returned series */
        REAL8 fMax,                     /**< maximum frequency of the returned series */
...
{
    Approximant approx = SpinTaylorT4LightScalar;
    int n = XLALSimInspiralSpinTaylorLightScalarDriverFourier(hplus, hcross, fMin, fMax,
            deltaF, kMax, phiRef, v0,
            m1, m2, fStart, fRef, r, s1x, s1y, s1z, s2x, s2y, s2z,
            lnhatx, lnhaty, lnhatz, e1x, e1y, e1z,
            eos_type, omegaBD, mScalar,
            lambda1, lambda2, quadparam1, quadparam2, spinO, tideO, phaseO, amplitudeO, approx, phiRefAtEnd, extraParams);
