...
//Function prototypes
INT4 XLALCGimri_SearchArr(REAL8 *arr, REAL8 time);
REAL8 XLALCGimri_InitialP(REAL8 p0, void *params);
INT4 XLALCGimri_ComputeEnergyAng(REAL8 p, REAL8 e, REAL8 a, INT4 dir, REAL8 *Eng, REAL8 *Ang);
REAL8 XLALCGimri_dFdr(REAL8 E, REAL8 Lz, REAL8 a, REAL8 r);
REAL8 XLALCGimri_dVphiVtdr(REAL8 E, REAL8 Lz, REAL8 a, REAL8 r);
REAL8 XLALCGimri_d2FdrdE(REAL8 E, REAL8 Lz, REAL8 a, REAL8 r);
REAL8 XLALCGimri_d2FdrdL(REAL8 E, REAL8 Lz, REAL8 a, REAL8 r);
REAL8 XLALCGimri_Edot3PN_Effective(REAL8 a, REAL8 p, REAL8 e, REAL8 nu);
REAL8 XLALCGimri_Ldot3PN_Effective(REAL8 a, REAL8 p, REAL8 e, REAL8 nu, INT4 dir);
REAL8 XLALCGimri_EnergyAngDot(REAL8 a, REAL8 p, REAL8 e, REAL8 nu, INT4 dir, REAL8* dEdt, REAL8* dLdt);
INT4 XLALCGimri_PEdot(REAL8 E, REAL8 Lz, REAL8 a, REAL8 p, REAL8 e, REAL8 nu, REAL8 dir, REAL8 *pdot,
	REAL8 *edot);
REAL8 XLALCGimri_dphidt_geo(REAL8 E, REAL8 Lz, REAL8 a, REAL8 r);
REAL8 XLALCGimri_dphidt_sf(REAL8 E, REAL8 Lz, REAL8 a, REAL8 p, REAL8 e, REAL8 psi, REAL8 nu);
REAL8 XLALCGimri_dpsidt_geo(REAL8 E, REAL8 Lz, REAL8 a, REAL8 p, REAL8 e, REAL8 psi);
REAL8 XLALCGimri_dpsidt_sf(REAL8 E, REAL8 Lz, REAL8 a, REAL8 p, REAL8 e, REAL8 psi, REAL8 nu);
REAL8 XLALCGimri_Plso(REAL8 a, REAL8 e, INT4 dir);
INT4 XLALCGimri_PlusEquations(const gsl_vector * x, void *params, gsl_vector * f);
INT4 XLALCGimri_CrossEquations(const gsl_vector * z, void *params, gsl_vector * g);
INT4 XLALCGimri_generator(
...
INT4 XLALCGimri_start(REAL8 m, REAL8 M, REAL8 a, REAL8 Dist, REAL8 Sdotn, REAL8 phi0, REAL8 psi0, REAL8 p0, REAL8 e0,
...
REAL8 XLALCGimri_InitialP(REAL8 p0, void *params) {
	...
	REAL8 E, L, nu;
	XLALCGimri_ComputeEnergyAng(p0,e0,a,dir,&E,&L);
        nu = (m*M)/pow(m+M,2.);

        return XLALCGimri_dphidt_sf(E,L,a,p0,e0,0.,nu) - pi*f0*(m+M)*Msun_sec;
...
REAL8 XLALCGimri_EnergyAngDot(REAL8 a, REAL8 p, REAL8 e, REAL8 nu, INT4 dir, REAL8* dEdt, REAL8* dLdt) {
	...
	REAL8 dLdt_0 = (XLALCGimri_Ldot3PN_Effective(a,p,0.,nu,dir) + Ldot_fit);
	*dLdt = pow(1.-e*e,3./2.)*(XLALCGimri_Ldot3PN_Effective(a,p,e,nu,dir) + Ldot_fit);
	...
	XLALCGimri_ComputeEnergyAng(p,0.,a,dir,&E0,&L0);

	REAL8 N1 = E0*pow(p,4.) + pow(a,2.)*E0*pow(p,2.) - 2.*a*(L0-a*E0)*p;
	REAL8 N2 = -L0*pow(p,2.) + 2.*(L0-a*E0)*p;

	*dEdt =  pow(1.-e*e,3./2.)*(XLALCGimri_Edot3PN_Effective(a,p,e,nu) - (N2/N1)*dLdt_0);

	return 0;

	}
	
INT4 XLALCGimri_PEdot(REAL8 E, REAL8 Lz, REAL8 a, REAL8 p, REAL8 e, REAL8 nu, REAL8 dir, REAL8 *pdot, REAL8 *edot) {
	...
	REAL8 dLdt, dEdt;
	XLALCGimri_EnergyAngDot(a,p,e,nu,dir,&dEdt,&dLdt);	
...
INT4 XLALCGimri_start(REAL8 m, REAL8 M, REAL8 a, REAL8 Dist, REAL8 Sdotn, REAL8 phi0, REAL8 psi0,
...
//	fprintf(logF,"arrays defined\n");
	...
	for (INT4 i=0; i<Npts; i++) {
		...
		switch (currentStage) {
			...
			case INSPIRAL:
				...
				XLALCGimri_ComputeEnergyAng(p,e,a,dir,&E,&L);
				
				//Get derivatives
				//dphidt[i] = XLALCGimri_dphidt_sf(E[i],L[i],a,p[i],e[i],psi[i],nu);
				//dpsidt[i] = XLALCGimri_dpsidt_sf(E[i],L[i],a,p[i],e[i],psi[i],nu);
				dphidt = XLALCGimri_dphidt_geo(E,L,a,r_Arr[i]);
				dpsidt = XLALCGimri_dpsidt_geo(E,L,a,p,e,psi);
				XLALCGimri_PEdot(E,L,a,p,e,nu,dir,&dpdt,&dedt);
				...
				XLALCGimri_ComputeEnergyAng(p_K2,e_K2,a,dir,&E_K2,&L_K2);
				XLALCGimri_PEdot(E_K2,L_K2,a,p_K2,e_K2,nu,dir,&dpdt_K2,&dedt_K2);
				//dphidt_K2 = XLALCGimri_dphidt_sf(E_K2,L_K2,a,p_K2,e_K2,psi_K2,nu);
				//dpsidt_K2 = XLALCGimri_dpsidt_sf(E_K2,L_K2,a,p_K2,e_K2,psi_K2,nu);
				dphidt_K2 = XLALCGimri_dphidt_geo(E_K2,L_K2,a,p_K2/(1.+e_K2*cos(psi_K2)));
				dpsidt_K2 = XLALCGimri_dpsidt_geo(E_K2,L_K2,a,p_K2,e_K2,psi_K2);

				//Get third order RK4 terms
				p_K3	= p+(dt/2.)*dpdt_K2;
				e_K3	= e+(dt/2.)*dedt_K2;
				psi_K3	= psi+(dt/2.)*dpsidt_K2;
				XLALCGimri_ComputeEnergyAng(p_K3,e_K3,a,dir,&E_K3,&L_K3);
				XLALCGimri_PEdot(E_K3,L_K3,a,p_K3,e_K3,nu,dir,&dpdt_K3,&dedt_K3);
				//dphidt_K3 = XLALCGimri_dpsidt_sf(E_K3,L_K3,a,p_K3,e_K3,psi_K3,nu);
				//dpsidt_K3 = XLALCGimri_dpsidt_sf(E_K3,L_K3,a,p_K3,e_K3,psi_K3,nu);
				dphidt_K3 = XLALCGimri_dphidt_geo(E_K3,L_K3,a,p_K3/(1.+e_K3*cos(psi_K3)));
				dpsidt_K3 = XLALCGimri_dpsidt_geo(E_K3,L_K3,a,p_K3,e_K3,psi_K3);

				//Get fourth order RK4 terms
				p_K4	= p+dt*dpdt_K3;
				e_K4	= e+dt*dedt_K3;
				psi_K4	= psi+(dt)*dpsidt_K3;
				XLALCGimri_ComputeEnergyAng(p_K4,e_K4,a,dir,&E_K4,&L_K4);
				XLALCGimri_PEdot(E_K4,L_K4,a,p_K4,e_K4,nu,dir,&dpdt_K4,&dedt_K4);
				//dphidt_K4 = XLALCGimri_dpsidt_sf(E_K4,L_K4,a,p_K4,e_K4,psi_K4,nu);
				//dpsidt_K4 = XLALCGimri_dpsidt_sf(E_K4,L_K4,a,p_K4,e_K4,psi_K4,nu);
				dphidt_K4 = XLALCGimri_dphidt_geo(E_K4,L_K4,a,p_K4/(1.+e_K4*cos(psi_K4)));
				dpsidt_K4 = XLALCGimri_dpsidt_geo(E_K4,L_K4,a,p_K4,e_K4,psi_K4);
				...
				p_lso = XLALCGimri_Plso(a,e+de,dir);
				...
				if (fabs(p-p_lso) <= 0.01) {
					...
					XLALCGimri_ComputeEnergyAng(p_lso,e,a,dir,&E_lso,&L_lso);
					XLALCGimri_EnergyAngDot(a,p_lso,e,nu,dir,&Edot_lso,&Ldot_lso);
					...
					i_match = XLALCGimri_SearchArr(t,t_lso + (-1.)*(dtdtau_lso*tau0*pow(nu,-1./5.)));
					t_match = t[i_match];
					r = r_Arr[i_match+1];
					...
					dphidt_old = XLALCGimri_dphidt_geo(E_lso+(t_match-t_lso)*Edot_lso, L_lso+(t_match-t_lso)*Ldot_lso,a,r);
			...
			******************************/
			case TRANSITION:
				...
				d2rdt2 = (1./2.)*(XLALCGimri_d2FdrdE(E_lso,L_lso,a,r)*(E - E_lso) + XLALCGimri_d2FdrdL(E_lso,L_lso,a,r)*(L - L_lso) + XLALCGimri_dFdr(E_lso,L_lso,a,r));
				dphidt = XLALCGimri_dphidt_geo(E,L,a,r);
				d2phidt2 = (dphidt-dphidt_old)/(t[i]-t[i-1]);
				...
				dphidt_K2 = XLALCGimri_dphidt_geo(E_K2,L_K2,a,r_K2);
				d2rdt2_K2 = (1./2.)*(XLALCGimri_d2FdrdE(E_lso,L_lso,a,r_K2)*(E_K2 - E_lso) + XLALCGimri_d2FdrdL(E_lso,L_lso,a,r_K2)*(L_K2 - L_lso) + XLALCGimri_dFdr(E_lso,L_lso,a,r_K2));
				...
				dphidt_K3 = XLALCGimri_dphidt_geo(E_K3,L_K3,a,r_K3);
				d2rdt2_K3 = (1./2.)*(XLALCGimri_d2FdrdE(E_lso,L_lso,a,r_K3)*(E_K3 - E_lso) + XLALCGimri_d2FdrdL(E_lso,L_lso,a,r_K3)*(L_K3 - L_lso) + XLALCGimri_dFdr(E_lso,L_lso,a,r_K3));
				...
				dphidt_K4 = XLALCGimri_dphidt_geo(E_K4,L_K4,a,r_K4);
				d2rdt2_K4 = (1./2.)*(XLALCGimri_d2FdrdE(E_lso,L_lso,a,r_K4)*(E_K4 - E_lso) + XLALCGimri_d2FdrdL(E_lso,L_lso,a,r_K4)*(L_K4 - L_lso) + XLALCGimri_dFdr(E_lso,L_lso,a,r_K4));
			...
			******************************/
			case PLUNGE:
				...
				dphidt = XLALCGimri_dphidt_geo(E_plunge,L_plunge,a,r);
				d2rdt2 = (1./2.)*XLALCGimri_dFdr(E_plunge,L_plunge,a,r);
				d2phidt2 = XLALCGimri_dVphiVtdr(E_plunge,L_plunge,a,r)*drdt;

				r_K2 = r + (dt/2.)*drdt;
				drdt_K2 = drdt + (dt/2.)*d2rdt2;
				dphidt_K2 = XLALCGimri_dphidt_geo(E_plunge,L_plunge,a,r_K2);
				d2rdt2_K2 = (1./2.)*XLALCGimri_dFdr(E_plunge,L_plunge,a,r_K2);

				r_K3 = r + (dt/2.)*drdt_K2;
				drdt_K3 = drdt + (dt/2.)*d2rdt2_K2;
				dphidt_K3 = XLALCGimri_dphidt_geo(E_plunge,L_plunge,a,r_K3);
				d2rdt2_K3 = (1./2.)*XLALCGimri_dFdr(E_plunge,L_plunge,a,r_K3);

				r_K4 = r + (dt)*drdt_K3;
				drdt_K4 = drdt + (dt)*d2rdt2_K2;
				//drdt_K4 = drdt + (dt)*d2rdt2_K3;
				dphidt_K4 = XLALCGimri_dphidt_geo(E_plunge,L_plunge,a,r_K4);
				d2rdt2_K4 = (1./2.)*XLALCGimri_dFdr(E_plunge,L_plunge,a,r_K4);
	...
	**************************************/
	...
	//Define functions
	gsl_multiroot_function f_plus = {&XLALCGimri_PlusEquations, nmatch, &par};
	gsl_multiroot_function f_cross = {&XLALCGimri_CrossEquations, nmatch, &par};
	...
	//Find root of CGimri_PlusEquations
	do {
		iter++;
	...
	//Find root of CGimri_CrossEquations
	iter = 0;
	do {
		iter++;
...
INT4 XLALCGimri_generator(
	...
	gsl_function F;
	F.function = &XLALCGimri_InitialP;
	F.params = &p0Params;

	REAL8 p0 = 0.;
	REAL8 p_high = 1000.;
	REAL8 p_low = XLALCGimri_Plso(a,e0,dir);
	...
	*hcross = XLALCreateREAL8TimeSeries("h_cross",&epoch,fDyne,dt,&lalStrainUnit,length);
	...
	//IMRIstart....
	XLALCGimri_start(m*Msun_sec, M*Msun_sec, a, dist*GPC_sec, Sdotn, phi0, psi0, p0, e0, (*hplus)->data, (*hcross)->data,
...
INT4 main(void) {
	...
	//INT4 finalInt = XLALCGimri_start(params,0.0,dt1/((m+M)*Msun_sec),dt2/((m+M)*Msun_sec),maxPts-1,t,h_plus,h_cross);
