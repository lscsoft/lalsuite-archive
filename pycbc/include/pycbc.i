...
%extend complex_fft_plan_t {
  complex_fft_plan_t(int size, int fwdflag, int level);
  ~complex_fft_plan_t();
  ...
  char *__str__() {
    ...
    {
      snprintf( a, sizeof(a)/sizeof(*a), 
          "<reverse complex_fft_plan of length %d>", self->size );
    }
    else
    {
      snprintf( a, sizeof(a)/sizeof(*a),
          "<forward complex_fft_plan of length %d>", self->size );
    }
    return a;
  }
}
