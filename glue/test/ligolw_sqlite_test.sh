...
# merge two .xml files into a single .xml document?
#

echo
echo "ligolw_sqlite test 1:  merge .xml.gz files and compare to ligolw_add"
echo "--------------------------------------------------------------------"
...
ligolw_sqlite --verbose --replace --database ${BASE}.sqlite --extract ${BASE}_output.xml ${BASE}_input.xml.gz ${BASE}_input.xml.gz
...
echo
echo "ligolw_sqlite test 2:  merge .sqlite files and compare to ligolw_add"
echo "--------------------------------------------------------------------"
ligolw_sqlite --verbose --preserve-ids --replace --database ${BASE}_input.sqlite ${BASE}_input.xml.gz
ligolw_sqlite --verbose --replace --database ${BASE}.sqlite --extract ${BASE}_output.xml ${BASE}_input.sqlite ${BASE}_input.sqlite
cmp ${BASE}_ref.xml ${BASE}_output.xml || exit
