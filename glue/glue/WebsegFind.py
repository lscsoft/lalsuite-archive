"""
The WebsegFind.py module contains Classes e.g., used by WebsegFind
"""

__author__ = "Greg Mendell: module that contains Classes e.g., used by WebsegFind"
__date__ = '$Date$'
...
try:
        ...
        print >>sys.stderr, "\nWARNING: Please try using LSCsegFind. WebsegFind will be deprecated in the future, and currently serves as a backup to LSCsegFind."
        print >>sys.stderr, "\nPython glue.WebsegFind library not found; please follow the instructions in the glue README file or add the location of glue to the PYTHONPATH environmental variable.\n"        
        ...
        print >>sys.stderr, "WARNING: Please try using LSCsegFind. WebsegFind will be deprecated in the future, and currently serves as a backup to LSCsegFind.\n"
        sys.exit(1)

class WebsegFindException(exceptions.Exception):
        """
        ...
        def __init__(self, args=None):
                """
                Create an instance of this class, ie. an WebsegFindClientException.

                @param args:

                @return: Instance of class WebsegFindClientException
                """
                print 'An exception was raised and handled in Class WebsegFind. See last message:'
                self.args = args
                

class WebsegFind(object):
        """
        ...
        saveFiles=False, cfgFile=None, localSegFile=None, coalesce=False, showTypes=False, curlPath="/usr/bin/curl"):
                """
                  See help for WebsegFind for definitions of parameters.
                """

                # Define defaults for local temporary files and local files to save if saveFiles is True.
                self.tmpCfgDefaultsFileName  = 'tmpCfgDefaults_WebsegFind.py'
                self.tmpCfgDefaultsFileNameC = 'tmpCfgDefaults_WebsegFind.pyc'
                self.savCfgDefaultsFileName  = 'savCfgDefaults_WebsegFind.py'
                self.tmpSegDataFileName = 'tmpSegData_WebsegFind.txt'
                self.savSegDataFileName = 'savSegData_WebsegFind.txt'
        ...
        def GetSegments(self):
                """
                   Get segments and print those that match WebsegFind input parameters to stdout
                """
