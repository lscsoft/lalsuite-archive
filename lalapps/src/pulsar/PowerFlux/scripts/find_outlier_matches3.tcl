...
set PI_INDEX  [lsearch -exact $HEADER "pi"]
...
while { ![eof $FILE] } {
	...
	set PI [lindex $s $PI_INDEX]
	set SNR [lindex $s $SNR_INDEX]

	if { [regexp "_all$" $SET] } {
		if { $SNR < $MIN_ALL_SNR } { continue }
		} {
		lappend SETS $SET
		if { $SNR < $MIN_GEN_SNR } { continue }
		}

	set idx [expr round([lindex $s $FREQUENCY_INDEX]/$FREQ_TOLERANCE)]
	lappend DATA(${SET}_${PI}_$idx) $s
	}
...
foreach tag [array names DATA -regexp "_all_"] {
	regexp {^(.*)_all_([^_]*)_(.*)} $tag {} ROOT PI BIN
	#puts "$ROOT $BIN"
	...
	foreach line $DATA($tag) {
		...
		foreach root $DET_ROOTS {
			...
			foreach bin [list $BIN [expr $BIN-1] [expr $BIN+1]] {
				if { ! [info exists DATA(${root}_${PI}_${bin})] } { continue }
				set found 1
				foreach s $DATA(${root}_${PI}_${bin}) {
...
puts stderr "Processed $FILENAME"
