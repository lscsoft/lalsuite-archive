...
void sse_get_uncached_single_bin_power_sum(SEGMENT_INFO *si, int count, PARTIAL_POWER_SUM_F *pps)
{
int i,k,n,m;
int bin_shift;
SEGMENT_INFO *si_local;
...
sse_get_uncached_single_bin_power_sum(SIMPLE_CACHE.si[k], SIMPLE_CACHE.segment_count, SIMPLE_CACHE.pps[k]);
sse_accumulate_partial_power_sum_F(pps, SIMPLE_CACHE.pps[k]);
}
...
/* reference implementation */
get_uncached_single_bin_power_sum(si, count, ps3);

/* sse implementation */
sse_get_uncached_single_bin_power_sum(si, count, ps4);
