...
#include "../GCT/HierarchSearchGCT.h"
...
typedef struct {
  ...
  BOOLEAN useResamp;		/**< use resampling version of CFS */
...
void SetUpSFTs( LALStatus *status, MultiSFTVectorSequence *stackMultiSFT,
               ...
               MultiDetectorStateSeriesSequence *stackMultiDetStates, UsefulStageVariables *in ); //Initialization for COMPUTEFSTATFREQBAND_RS
...
/* write_FstatCandidate_to_fp uses ifreq and dFreqStack in the resampling case
 * If useResamp is not selected, ifreq = 0 and
 * write_FstatCandidate_to_fp reverts to its default behavior */
int write_FstatCandidate_to_fp ( FILE *fp, const FstatCandidate *thisFCand, UINT4 ifreq, REAL8 dFreqStack );
...
int main(int argc,char *argv[])
{
  ...
  if ( uvar.useResamp ) {
    /*Initialize usefulParams and call SetUpSFTs*/
    /* some useful first stage params */
    usefulParams.sftbasename = uvar.DataFiles;
    usefulParams.nStacks = 1; //all data in one "stack" since CFS doesn't use stacks
    ...
    usefulParams.spinRange_refTime.fkdot[0] = uvar.Freq; /* frequency */
    usefulParams.spinRange_refTime.fkdot[1] = uvar.f1dot;  /* 1st spindown */
    usefulParams.spinRange_refTime.fkdotBand[0] = uvar.FreqBand; /* frequency range */
    usefulParams.spinRange_refTime.fkdotBand[1] = uvar.f1dotBand; /* spindown range */

    usefulParams.edat = GV.ephemeris;
    XLALGPSSetREAL8(&usefulParams.minStartTimeGPS, uvar.minStartTime);
    XLALGPSSetREAL8(&usefulParams.maxEndTimeGPS, uvar.maxEndTime);
    usefulParams.blocksRngMed = uvar.RngMedWindow;
    usefulParams.Dterms = uvar.Dterms;
    usefulParams.SignalOnly = uvar.SignalOnly;
    usefulParams.dopplerMax = uvar.dopplermax;
    ...
    dFreqStack = 1.0/(2*tObs);	//To allow ComputeFstat_RS to handle freq. range
				//uvar.dFreq is set to be the freq. range + some small epsilon
  ...
  while ( !uvar.countTemplates && (XLALNextDopplerPos( &dopplerpos, GV.scanState ) == 0) )
    {
      ...
      if ( uvar.useResamp ) {
        ...
        dopplerpos.refTime = tMidGPS; /* midpoint of data spanned */
      } /*--the resampling code sets the reference time at the midpoint--*/
	  ...
	  CFparams.buffer = resampbuffers.data[k];
          ...
          LAL_CALL( COMPUTEFSTATFREQBAND_RS ( &status, &fstatVector.data[k], &dopplerpos, stackMultiSFT.data[k], stackMultiNoiseWeights.data[k], &CFparams), &status);
      ...
      if ( lalDebugLevel && ( ++tickCounter > uvar.timerCount) )
	{
      ...
      for (ifreq = 0; ifreq < fveclength; ifreq++) {    //Second for loop for the resampling case
                                                        //Compensates for the fact that the resampling code uses
                                                        //ComputeFstatFreqBand and thus produces more than one Fstat value
                                                        //per template.  Like the previous loop, its arguments are structured
                                                        //such that if useResamp is not selected, it will only execute once,
                                                        //mirroring the non-resampling case.
      ...
      if ( !finite(Fstat.F) )
	{
      ...
      if ( uvar.useResamp )
        {
        /* skip this step; ComputeFStat_RS.c doesn't preserve this information at present */
        /* NOTE: this means that --useResamp is not currently compatible with --outputFstatAtoms! */
        }
      else if ( !uvar.GPUready )
        {
      ...
      if ( uvar.SignalOnly )
	{
	  ...
	  if ( GV.FstatToplist  )			/* dynamic threshold */
	    ...
	    {
	      if ( write_FstatCandidate_to_fp ( fpFstat, writeCand, ifreq, dFreqStack ) != 0 )
		{
  ...
  if ( fpFstat && GV.FstatToplist )
      ...
      for ( el=0; el < GV.FstatToplist->elems; el ++ )
	{
	  ...
	  if ( write_FstatCandidate_to_fp ( fpFstat, candi, ifreq, dFreqStack ) != 0 )
	    {
	      LogPrintf (LOG_CRITICAL, "Failed to write candidate to file.\n");
	      return -1;
	    }
  ...
  if (uvar.useResamp) {
    /* free first stage memory--resampling */
    for (k=0; k<nStacks; k++) {
...
#define DEFAULT_EPHEMDIR "env LAL_DATA_PATH"
  ...
  uvar->GPUready = 0;
  uvar->useResamp = 0;		/* by default do not use resampling */
...
#define DEFAULT_TRANSIENT "none"
  ...
  LALregBOOLUserStruct(status,  useResamp,        0,  UVAR_OPTIONAL,  "Use resampling version of core routines");
...
//Initialization Code for COMPUTEFSTATFREQBAND_RS
		...
		UsefulStageVariables *in /**< input params */)
{
  ...
  TRY( SetUpStacks( status->statusPtr, &catalogSeq, in->tObs, catalog, in->nStacks), status);
  ...
  freqLo = MYMIN ( startTime_freqLo, endTime_freqLo );
  freqHi = MYMAX ( startTime_freqHi, endTime_freqHi );
  doppWings = freqHi * in->dopplerMax;    /* maximum Doppler wing -- probably larger than it has to be */
  extraBins = MYMAX ( in->blocksRngMed/2 + 1, in->Dterms );
...
void SetUpStacks(LALStatus *status,	   /**< pointer to LALStatus structure */
		 ...
		 REAL8 tObs,               /**< Output duration of observation */
		 ...
		 UINT4 nStacksMax )        /**< User specified number of stacks */
{
  ...
  ASSERT ( in != NULL, status, HIERARCHICALSEARCH_ENULL, HIERARCHICALSEARCH_MSGENULL );
  ASSERT ( tObs > 0, status, HIERARCHICALSEARCH_ENULL, HIERARCHICALSEARCH_MSGENULL );
 ...
 */
int
write_FstatCandidate_to_fp ( FILE *fp, const FstatCandidate *thisFCand, UINT4 ifreq, REAL8 dFreqStack )
{
  ...
  fprintf (fp, "%.16g %.16g %.16g %.6g %.5g %.5g %.9g%s\n",
	   thisFCand->doppler.fkdot[0]+ifreq*dFreqStack, thisFCand->doppler.Alpha, thisFCand->doppler.Delta,
