...
#generate dag
/bin/echo -n "Generating ifar_result_not_combined_${data_type}.dag and .sub files..."
if [ 1 ]; then
  ...
  for mass in mchirp_2_8 mchirp_8_17 mchirp_17_35; do
    ...
    for combo in H1L1 H2L1 H1H2L1; do
      ...
      echo "JOB $job_name ifar_result_not_combined_${data_type}.plotifar.sub"
      echo "RETRY $job_name 1"
      echo "VARS $job_name macroglob=\"$glob_file\" macrooutpath=\"$outpath\" macroifotimes=\"$combo\" macrotriggers=\"$triggers\" macrousertag=\"$user_tag\" macrotcorrnum=\"$t_correct_num\" macrotcorrdenom=\"$t_correct_denom\""
  ...
  for combo in H1L1 H2L1 H1H2L1; do
    ...
    echo "JOB $job_name ifar_result_not_combined_${data_type}.plotifar.sub"
    echo "RETRY $job_name 1"
    echo "VARS $job_name macroglob=\"$glob_file\" macrooutpath=\"$outpath\" macroifotimes=\"$combo\" macrotriggers=\"$triggers\" macrousertag=\"$data_type\" macrotcorrnum=\"$t_correct_num\" macrotcorrdenom=\"$t_correct_denom\""
    echo "CATEGORY $job_name plotifar"
...
fi > ifar_result_not_combined_${data_type}.dag
...
if [ 1 ]; then
  ...
  echo "arguments = --glob \$(macroglob) --output-path \$(macrooutpath) --ifo-times \$(macroifotimes) \$(macrotriggers) --gps-start-time ${month_gps_time} --gps-end-time ${gps_end_time} --ifar-dist --enable-output --user-tag \$(macrousertag)_NOT_COMBINED --t-correct-num \$(macrotcorrnum) --t-correct-denom \$(macrotcorrdenom)"
  echo "getenv = True"
  echo "log = " `mktemp -p ${log_path}`
  ...
  echo "priority = ${condor_priority}"
  echo "queue 1"
fi > ifar_result_not_combined_${data_type}.plotifar.sub
...
echo " done."
echo "*************************************************************"
echo "  Now run: condor_submit_dag ifar_result_not_combined_${data_type}.dag"
echo "*************************************************************"
