...
/* Modify the both the noise datastream and PSD */
    int IFOnum=0;
    UINT4 j;
    REAL8 fstart = 1.0;
    REAL8 fend = 4000.0;
    ...
    for(j=0;j<inputMCMC.invspec[i]->data->length;j++){
        ...
        //else
         if(enable_calfreq){ inputMCMC.invspec[i]->data->data[j]/=((ConvertCoefficientsToFunction(ampFit,j*inputMCMC.deltaF,cen)*ConvertCoefficientsToFunction(ampFit,j*inputMCMC.deltaF,cen)));}
    }
...
void ApplyCalibrationErrorsToWaveform(COMPLEX16FrequencySeries *injF,COMPLEX16FrequencySeries *CalibInj,CHAR *IFOname, int seed){
    UINT4 j;
    REAL8 fstart = 1.0;
    REAL8 fend = 4000.0;
    INT4 length= 13;
