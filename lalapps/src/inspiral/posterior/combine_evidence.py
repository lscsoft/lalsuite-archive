...
def nest2pos(samps,weights,logl_index=-1):
    randoms=rand(size(samps,0))
    wt=weights+samps[:,logl_index]
    maxwt=max(wt)
...
def combine_evidence(data,xflag,Nlive,logl_index=-1):
    ...
    pos=nest2pos(d_all,weights,logl_index)

    d_idx=argsort(d_all[:,logl_index])
    d_all=d_all[d_idx,:]
