...
(--no-logdistance)\tUse distance, not logdistance, as the sampling variable\n";
	...
	if(LALInferenceGetProcParamVal(commandLine,"--single-spin")) {
	  enable_spin=1;
	  single_spin=1;
	}
	...
	if(enable_spin){
		...
		if(aligned_spin){ /* Set the spin angles to be parallel to orbital */
		  ...
		  REAL8 tmpValT=theta_spin1_min+(theta_spin1_max - theta_spin1_min)/2.0;
		  if(!LALInferenceCheckVariable(currentParams,"theta_spin1")) 
		    LALInferenceAddVariable(currentParams,"theta_spin1",	&tmpValT,	LALINFERENCE_REAL8_t, LALINFERENCE_PARAM_LINEAR);
		  ...
		  if (!single_spin) {
		    if(!LALInferenceCheckVariable(currentParams,"theta_spin2")) 
		      LALInferenceAddVariable(currentParams,"theta_spin2",	&tmpValT,	LALINFERENCE_REAL8_t, LALINFERENCE_PARAM_LINEAR);
		    ...
		    if(!LALInferenceCheckVariable(currentParams,"phi_spin2")) 
		      LALInferenceAddVariable(currentParams,"phi_spin2",		&tmpValP,	LALINFERENCE_REAL8_t, LALINFERENCE_PARAM_CIRCULAR);
