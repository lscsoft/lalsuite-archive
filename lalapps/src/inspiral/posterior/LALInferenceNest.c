...
//(--tdlike)\tUse time domain likelihood.\n";
  ...
  if(ppt) {
    if(XLALCheckBurstApproximantFromString(ppt->value) || !strcmp(ppt->value,"PrincipalComp") || !strcmp(ppt->value,"PrincipalCompBBH") || !strcmp(ppt->value,"PrincipalCompTime"))
  ...
  ppt=LALInferenceGetProcParamVal(commandLine,"--approx");
  if(ppt) {
    if(!strcmp("PrincipalComp",ppt->value) || !strcmp("SineGaussianF",ppt->value) || !strcmp("SineGaussian",ppt->value)|| !strcmp("Gaussian",ppt->value)|| !strcmp("GaussianF",ppt->value) || !strcmp("DampedSinusoid",ppt->value)|| !strcmp("DampedSinusoidF",ppt->value) || !strcmp(ppt->value,"PrincipalCompBBH") || !strcmp(ppt->value,"PrincipalCompTime")){
    ...
    if (XLALCheckBurstApproximantFromString(ppt->value)){
      ...
      initModelFunc=&LALInferenceInitModelReviewEvidence_banana;
	    ...
	    initModelFunc=&LALInferenceInitBurstModel;
            ...
            fprintf(stdout,"--- Calling PCA time init variables \n");
            initModelFunc=&LALInferenceInitPrincipalCompTimeModel;
        }
	else{
