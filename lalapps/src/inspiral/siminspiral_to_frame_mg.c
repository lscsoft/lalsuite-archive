...
static struct options options_defaults(void)
{
   ...
   defaults.fref=-1.;
   defaults.loglambda_g=-1.;
    return defaults;
}

static void ParseCharacterOptionString(char *input, char **strings[], UINT4 *n)
...
"[--channels [A,B,..,N]: Override the default suffix of the channels to be IFO1:A, IFO2:B, etc. \n" \
"[--pad seconds]: Override the default padding before the first and after the last signals (this is neglected if gps-start is given)\n"\
"[--loglambda_g]: Inject a nonGR value of loglambda_g (default GR) \n" \
"[--fref] Reference frequency for precessing approximants (default f_lower)\n"\
...
static struct options parse_command_line(int *argc, char **argv[])
{
    ...
    {"fref", required_argument,NULL,1990},
    {"loglambda_g", required_argument,NULL,1991},
    {NULL, 0, NULL, 0}
	};
  ...
  case 1990:
    options.fref=atof(optarg);
  case 1991:
    options.loglambda_g=atof(optarg);
...
int main(int argc, char **argv)
{
  ...
  if (options.fref!=-1.)
    fref=options.fref;
  REAL8 loglambda_g=-1;
  if (options.loglambda_g!=-1.)
    loglambda_g=options.loglambda_g;
   ...
   * The time series is saved in the frame calling XLALFrameAddREAL8TimeSeriesSimData */
	for (i=0;i<options.nIFO;i++){
		...
		if (loglambda_g!=-1.)
		  XLALSimInspiralAddTestGRParam(&nonGRparams,"loglambda_g",loglambda_g);
		
		REAL8 lambda1=0.0;
                REAL8 lambda2=0.0;
		LALPNOrder amp_order = (LALPNOrder) inj->amp_order;
		LALPNOrder phase_order = XLALGetOrderFromString(inj->waveform);
		Approximant approximant = XLALGetApproximantFromString(inj->waveform);
		REAL8 f_min = inj->f_lower;//fLow2fStart(inj->f_lower, amp_order, approximant);
		
		XLALSimInspiralTD(&hplus, &hcross, inj->coa_phase, deltaT,
                                  inj->mass1*LAL_MSUN_SI, inj->mass2*LAL_MSUN_SI, inj->spin1x,
                                  inj->spin1y, inj->spin1z, inj->spin2x, inj->spin2y,
                                  inj->spin2z, f_min, fref, inj->distance*LAL_PC_SI * 1.0e6, 0.0,
                                  inj->inclination, lambda1, lambda2, waveFlags,
                                  nonGRparams, amp_order, phase_order, approximant);

	        XLALSimInspiralDestroyTestGRParam(nonGRparams);
		
		LALDetector *detector=XLALCalloc(1,sizeof(LALDetector));
		if(!strcmp(options.ifonames[i],"H1")) 
		  memcpy(detector,&lalCachedDetectors[LALDetectorIndexLHODIFF],sizeof(LALDetector));
		if(!strcmp(options.ifonames[i],"H2")) 
		  memcpy(detector,&lalCachedDetectors[LALDetectorIndexLHODIFF],sizeof(LALDetector));
		if(!strcmp(options.ifonames[i],"LLO")||!strcmp(options.ifonames[i],"L1")) 
		  memcpy(detector,&lalCachedDetectors[LALDetectorIndexLLODIFF],sizeof(LALDetector));
		if(!strcmp(options.ifonames[i],"V1")||!strcmp(options.ifonames[i],"VIRGO")) 
		  memcpy(detector,&lalCachedDetectors[LALDetectorIndexVIRGODIFF],sizeof(LALDetector));
              memcpy(&(hplus->epoch),&inj->geocent_end_time,sizeof(LIGOTimeGPS));
              memcpy(&(hcross->epoch),&inj->geocent_end_time,sizeof(LIGOTimeGPS));
              XLALGPSAdd(&(hplus->epoch),-(deltaT*hplus->data->length));
              XLALGPSAdd (&(hcross->epoch),-(deltaT*hcross->data->length));
              printf("%d %d \n",soft->epoch.gpsSeconds,hplus->epoch.gpsSeconds);
	      XLALSimInjectDetectorStrainREAL8TimeSeries(soft,hplus,hcross,inj->longitude,inj->latitude,inj->polarization, detector, NULL);
