...
int FindEvents(struct CommandLineArgsTag CLA, const StringTemplate *strtemplate, const REAL8TimeSeries *vector, SnglBurst **head){
  ...
  if (CLA.printsnrflag){
    for ( p = vector->data->length/4 ; p < 3*vector->data->length/4; p++ )
      fprintf(stdout,"%d %.5f %e\n", strtemplate->findex, vector->epoch.gpsSeconds+10e-9*vector->epoch.gpsNanoSeconds+p*vector->deltaT, vector->data->data[p]);
  }
...
int ReadCommandLine(int argc,char *argv[],struct CommandLineArgsTag *CLA, const ProcessTable *process, ProcessParamsTable **paramaddpoint){
  ...
  while ( 1 )
  {
    ...
    case 'h':
      ...
      fprintf(stdout,"\t--settling-time (-T)\t\tREAL8\t Number of seconds to truncate filter.\n");
      ...
      fprintf(stdout,"\t--print-td-filter (-x)\tFLAG\t Prints the time domain filter to FIRFilter-<template no>.txt.\n");
