...
#define PROGRAM_NAME "coherent_ringdown"
...
INT4   numPointsSeg         = -1;/* set to segment-length used in ring.c */
REAL4  fLow                 = -1;       /* low frequency cutoff         */
REAL4  dynRangeExponent     = -1;/* set to same value used in ring.c */
...
REAL4  cohSegLength     = 0.0625; /* This should match value (sec) used in ring.c */
...
int main( int argc, char *argv[] )
{
  ...
  SnglRingdownTable    *cohbankEventList=NULL;
  ...
  numTriggers = XLALReadRingdownTriggerFile( &cohbankEventList,
  ...
  if ( numTriggers < 0 )
    ...
    numCoincs = XLALRecreateRingdownCoincFromSngls( &coincHead, cohbankEventList );
	...
	CHAR   caseIDChars[6][LIGOMETA_IFOS_MAX] = {"0","0","0","0","0","0"};
        ...
        for( k=0 ; k<LAL_NUM_IFO ; k++)
          {
            if( thisCoinc->snglRingdown[k] )
              {
                int temp_frequency = 0;
                int temp_quality = 0;
                kmax = k; /* final trigger's k value */
                caseID[k] = 1;
                memcpy( caseIDChars[k], &thisCoinc->snglRingdown[k]->ifo, sizeof(caseIDChars[k] - 1) );
                snprintf( channelNameArray[k], LALNameLength, "%s", thisCoinc->snglRingdown[k]->channel );
                eventID = thisCoinc->snglRingdown[k]->event_id->id;
                if( vrbflg ) fprintf(stdout,"eventID = %" LAL_UINT8_FORMAT "\n",eventID );
                chisq[l] = 1.0;
                chisq_dof[l] = 1.0;
                sigmasq[l] = thisCoinc->snglRingdown[k]->sigma_sq;
                if( vrbflg ) fprintf(stdout, "SIGMA_SQ = %f\n", sigmasq[l] );
                snrsqArray[l] = pow( (REAL8) thisCoinc->snglRingdown[k]->snr,2);
                ...
                */
                temp_frequency = floor( thisCoinc->snglRingdown[k]->frequency * 1000.0 );
                temp_quality = floor( thisCoinc->snglRingdown[k]->quality * 1000.0 );
                snprintf( nameArrayCData[k], LALNameLength*sizeof(CHAR),
                "%s:CBC-CData_%d_%d_%d_%d", caseIDChars[k],
                thisCoinc->snglRingdown[k]->start_time.gpsSeconds,
                (thisCoinc->snglRingdown[k]->start_time.gpsNanoSeconds -
                (thisCoinc->snglRingdown[k]->start_time.gpsNanoSeconds % 1000000))/1000000,
                temp_frequency, temp_quality );

                /* slideSign=0 is the same as a positive time slide */
                ifoNumber = XLALIFONumber(thisCoinc->snglRingdown[k]->ifo);
                if(slideSign != 0)
                  {
        ...
        nullStatRegul = XLALCoincRingdownStat( thisCoinc, coincStat, &bittenLParams);
	...
	if ( (numDetectors == 4) ) threeSiteCase = 1;
        ...
        /* CHECK: This mass section should be removed in the next version */
        m1 = 1.4;
        m2 = 1.4;
        muMass =  m1 * m2 / (m1 + m2);
        ...
        l = 0;
        for( k=0 ; k<LAL_NUM_IFO ; k++) {
          if( thisCoinc->snglRingdown[k] && (l == 0) ) {
            l=1;
            /* CHECK: Hard-wired value; should be replaced in the next version */
            cohInspFilterParams->chirpTime = 1.0;
          }
        }
...
#define USAGE1 \
"lalapps_coherent_ringdown [options]\n\n"\
...
"  --segment-length N           set N to same value used in ring.c\n"\
"  --dynamic-range-exponent N   set N to same value used in ring.c\n"\
   ...
   if ( numPointsSeg < 0 )
     {
       ...
       fprintf( stderr,"It must be set to the same value as was used in ring.c when the C-data was generated.\n");
       exit( 1 );
     }
