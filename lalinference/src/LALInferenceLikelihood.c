...
static void extractDimensionlessVariableVector(LALInferenceVariables *currentParams, REAL8 *x) {
  ...
  x[0] = (m1 - 16.0);
  x[1] = (m2 - 7.0);
  x[2] = (iota - M_PI/2.0)/(M_PI/20.0);
  x[3] = (phi - M_PI)/(M_PI/10.0);
  x[4] = (psi - M_PI/2.0)/(M_PI/20.0);
  x[5] = (ra - M_PI)/(M_PI/10.0);
  x[6] = dec/(M_PI/10.0);
  x[7] = (d-50.0)/10.0;
  x[8] = t/0.1;
  x[9] =(a1-0.5)/0.1;
  x[10]=(a2-0.5)/0.1;
  x[11]=(theta1-M_PI/2)/(M_PI/20.0);
  x[12]=(theta2-M_PI/2)/(M_PI/20.0);
  x[13]=(phi1-M_PI)/(M_PI/10.0);
  x[14]=(phi2-M_PI)/(M_PI/10.0);  
}
