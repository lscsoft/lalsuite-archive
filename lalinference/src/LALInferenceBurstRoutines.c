...
/* sigma is the width of the gaussian envelope in the freq domain WF ~ exp(-1/2 X^2/sigma^2)*/
  ...
  for(i = lower; i < upper; i++) {
    f=((REAL8 ) i )*deltaF;
    hptilde->data->data[i]  = -h0plus * (lambda*cp+  twopif*sp+LAL_TWOPI*1*cp*f)/(4.*LAL_PI*LAL_PI*f*f - 4.*LAL_PI*1*f*lambda-lambda2-omega2);
    hctilde->data->data[i]  = -h0cross * (-lambda*sp + twopif*cp - LAL_TWOPI*1*sp*f)/(4.*LAL_PI*LAL_PI*f*f - 4.*LAL_PI*1*f*lambda-lambda2-omega2);
  }
