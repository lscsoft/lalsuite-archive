...
# check environment
if (!defined($ENV{CPP})) {
    print "$0: needs environment variable 'CPP'\n";
    exit 1;
}
...
if ($verbose) {
    print  "$ifacefile:\n";
    printf "% 8i lines of header code\n", $numlines;
    printf "% 8i lines containing 'SWIGLAL' (%0.1f%%)\n", $numswiglines, 100.0 * $numswiglines / $numlines;
    foreach (sort { $a cmp $b } keys %numswigmacros) {
        printf "% 8i use(s) of '$_' macro\n", $numswigmacros{$_};
    }
}
