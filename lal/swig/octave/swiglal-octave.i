...
// (as of SWIG 2.0.3). This empty definition is to prevents warnings.
%runtime %{
  void Swig::swig_register_director(octave_swig_type*, void*, Swig::Director*) {}
%}

// octcomplex.swg is broken for single-precision complex numbers
// (as of SWIG 2.0.3). A fixed has been submitted (SF#3285699); until
// this makes its way into SWIG, we provide the fixed version here.
//
...
// Returns whether an octave_value is a valid, allocated object.
// Since octave_values are not pointers, this is always true.
%header %{
  SWIGINTERNINLINE bool
  swiglal_is_valid(const octave_value& v) {
    return true;
  }
%}
...
%header %{
  ...
  SWIGINTERNINLINE bool
  swiglal_is_vector(const octave_value& v) {
    return v.ndims() == 2 &&
  ...
  swiglal_vector_length(const octave_value& v) {
    return v.length();
  }

  // Get the (i)th element of an octave_value vector.
  SWIGINTERNINLINE octave_value
  swiglal_vector_get(octave_value v, const size_t i) {
  ...
  SWIGINTERNINLINE bool
  swiglal_vector_set(octave_value& v, const size_t i, octave_value vi) {
    std::list<octave_value_list> idx(1);
    idx.front()(0) = i + 1;
...
%header %{
  ...
  SWIGINTERNINLINE bool
  swiglal_is_matrix(const octave_value& v) {
    return v.ndims() == 2 &&
      v.rows() >= 1 && v.columns() >= 1;
  }
  ...
  SWIGINTERNINLINE size_t
  swiglal_matrix_rows(const octave_value& v) {
    return v.rows();
  }

  // Return the number of columns of an octave_value matrix.
  SWIGINTERNINLINE size_t
  swiglal_matrix_cols(const octave_value& v) {
    return v.columns();
  }

  // Get the (i,j)th element of an octave_value matrix.
  SWIGINTERN octave_value
  swiglal_matrix_get(octave_value v, const size_t i, const size_t j) {
    std::list<octave_value_list> idx(1);
    idx.front()(0) = i + 1;
    idx.front()(1) = j + 1;
    return v.subsref(v.is_cell() ? "{" : "(", idx);
  ...
  SWIGINTERN bool
  swiglal_matrix_set(octave_value& v, const size_t i, const size_t j, octave_value vij) {
    std::list<octave_value_list> idx(1);
    idx.front()(0) = i + 1;
    idx.front()(1) = j + 1;
...
%typemap(in) tm* (struct tm temptm) {
  ...
  if (!$input.is_empty()) {
    ...
    temptm.tm_year  = octtm(0) - 1900;   // 'tm' struct years start from 1900
    temptm.tm_mon   = octtm(1) - 1;      // 'tm' struct months start from 0
    temptm.tm_mday  = octtm(2);
    temptm.tm_hour  = octtm(3);
    temptm.tm_min   = octtm(4);
    temptm.tm_sec   = octtm(5);
    temptm.tm_isdst = octtm.numel() > 8 ? octtm(8) : -1;
