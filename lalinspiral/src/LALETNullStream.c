...
REAL8FrequencySeries * ComputeSingleDetectorInvPSDfromNullStream(REAL8TimeSeries *NullStream, REAL8 duration, REAL8 SampleRate, UINT4 nSegs){
	...
	REAL8FrequencySeries *inverse_spectrum = XLALCreateREAL8FrequencySeries("inverse spectrum",&NullStream->epoch,0.0,deltaF,&lalDimensionlessUnit,seglen/2+1);

	/***************************************************************************
	 *
	 *  RESAMPLE NULLSTREAM IF REQUESTED
	 * 
	 **************************************************************************/

	// RESAMPLE TIMESERIES - OPTIONAL
	if (SampleRate!=(1.0/NullStream->deltaT)) {
		fprintf(stdout,"... Sample rate %f, resampling...\r",SampleRate);
	...
	// NORMALIZE, ACCORDING TO EQN 35 FROM ET MDC PAPER 
	//NB: EQN35 IS FOR THE SPECTRAL DENSITY, BUT THE INVERSE SPECTRAL DENSITY IS CALCULATED HERE
