...
void XLALSQTPNFillCoefficients_Old(LALSQTPNWaveformParams * const params) {
	...
	case LAL_PNORDER_TWO_POINT_FIVE:
		params->coeff.domega[LAL_PNORDER_TWO_POINT_FIVE] = -(4159. + 15876. * params->eta) * LAL_PI
				/ 672.;
	...
	case LAL_PNORDER_ONE_POINT_FIVE:
		...
		if (params->spinInteraction != 0) {
			for (i = 0; i < 2; i++) {
				...
				params->coeff.mecoSO[i] = -spin_MPow2[i] * 5. * params->eta * (4. + 3. * m_m[i])
						/ 9.;
			}
		}
...
int LALSQTPNDerivator_Old(REAL8 t, const REAL8 values[], REAL8 dvalues[], void * param) {
	...
	UNUSED(t);
	const REAL8 *chi_p[2] = { values + LALSQTPN_OLD_CHIH1_1, values + LALSQTPN_OLD_CHIH2_1 };
	UINT2 i, j, k; // indexes
	memset(dvalues, 0, LALSQTPN_OLD_NUM_OF_VAR * sizeof(REAL8));
	REAL8 omegaPowi_3[8];
	omegaPowi_3[0] = 1.;
	omegaPowi_3[1] = cbrt(values[LALSQTPN_OLD_OMEGA]);
	for (i = 2; i < 8; i++) {
	...
	for (i = 0; i < 2; i++) {
		LNhchih[i] = SCALAR_PRODUCT3(values + LALSQTPN_OLD_LNH_1, chi_p[i]);
		VECTOR_PRODUCT3(values + LALSQTPN_OLD_LNH_1, chi_p[i], LNhxchih[i]);
	}

	// calculating domega and MECO without the spin components
	for (i = LAL_PNORDER_NEWTONIAN; i <= params->order; i++) {
		dvalues[LALSQTPN_OLD_OMEGA] += params->coeff.domega[i] * omegaPowi_3[i];
	}
	dvalues[LALSQTPN_OLD_MECO] += params->coeff.meco[0] / omegaPowi_3[1];
	for (i = LAL_PNORDER_NEWTONIAN + 2; i <= params->order; i += 2) {
		dvalues[LALSQTPN_OLD_MECO] += params->coeff.meco[i] * omegaPowi_3[i - 1];
	}
	...
	case LAL_PNORDER_THREE_POINT_FIVE:
	case LAL_PNORDER_THREE:
		dvalues[LALSQTPN_OLD_OMEGA] += params->coeff.domegaLN * log(16. * omegaPowi_3[2])
	...
	case LAL_PNORDER_TWO:
		if ((params->spinInteraction & LAL_SSInter) == LAL_SSInter) {
			...
			// SS for MECO
			dvalues[LALSQTPN_OLD_MECO] += params->coeff.mecoSS * (chih1chih2 - 3 * LNhchih[0]
			...
			for (i = 0; i < 2; i++) {
				...
				for (j = 0; j < 3; j++) {
					...
					dvalues[LALSQTPN_OLD_CHIH1_1 + 3 * i + j] += params->coeff.dchihSS[i]
		...
		if ((params->spinInteraction & LAL_QMInter) == LAL_QMInter) {
			...
			for (i = 0; i < 2; i++) {
				...
				for (j = 0; j < 3; j++) {
					// the 3*index is used, to acces the first spin, if index=0,
					// otherwise the second spin
					dvalues[LALSQTPN_OLD_CHIH1_1 + 3 * i + j] += params->coeff.dchihQM[i] * LNhchih[i]
							* LNhxchih[i][j] * omegaPowi_3[6];
				}
			}
			// QM for MECO
			dvalues[LALSQTPN_OLD_MECO] += params->coeff.mecoQM * QM_Omega * omegaPowi_3[3];
		}
		dvalues[LALSQTPN_OLD_OMEGA] += (QM_Omega + SSself_Omega + SS_Omega)
				* omegaPowi_3[LAL_PNORDER_TWO];
	case LAL_PNORDER_ONE_POINT_FIVE:
		if (params->spinInteraction != 0) {
			// SO for domega and MECO
			for (i = 0; i < 2; i++) {
				dvalues[LALSQTPN_OLD_OMEGA] += params->coeff.domegaSO[i] * LNhchih[i]
						* omegaPowi_3[LAL_PNORDER_ONE_POINT_FIVE];
				dvalues[LALSQTPN_OLD_MECO] += params->coeff.mecoSO[i] * LNhchih[i] * omegaPowi_3[2];
			}
			// dLNh and SO for dchih
			for (i = 0; i < 3; i++) {
				dvalues[LALSQTPN_OLD_CHIH1_1 + i] += params->coeff.dchihSO[0] * LNhxchih[0][i]
						* omegaPowi_3[5];
				dvalues[LALSQTPN_OLD_CHIH2_1 + i] += params->coeff.dchihSO[1] * LNhxchih[1][i]
						* omegaPowi_3[5];
				dvalues[LALSQTPN_OLD_LNH_1 + i] += (params->coeff.dLNh[0] * dvalues[LALSQTPN_OLD_CHIH1_1
						+ i] + params->coeff.dLNh[1] * dvalues[LALSQTPN_OLD_CHIH2_1 + i])
						* omegaPowi_3[1];
			}
		}
	case LAL_PNORDER_ONE:
	...
	dvalues[LALSQTPN_OLD_OMEGA] *= params->coeff.domegaGlobal * omegaPowi_3[7] * omegaPowi_3[4];
	REAL8 alphadotcosi, LNhxy = SQT_SQR(values[LALSQTPN_OLD_LNH_1]) + SQT_SQR(values[LALSQTPN_OLD_LNH_2]);
	if (LNhxy > 0.0) {
		alphadotcosi = values[LALSQTPN_OLD_LNH_3] * (values[LALSQTPN_OLD_LNH_2] * dvalues[LALSQTPN_OLD_LNH_1]
				- values[LALSQTPN_OLD_LNH_1] * dvalues[LALSQTPN_OLD_LNH_2]) / LNhxy;
	} else {
		alphadotcosi = 0.0;
	}
	dvalues[LALSQTPN_OLD_PHASE] = values[LALSQTPN_OLD_OMEGA] + alphadotcosi;
	return GSL_SUCCESS;
}
...
void LALSQTPNGenerator_Old(LALStatus *status, LALSQTPNWave *waveform, LALSQTPNWaveformParams *params) {
	...
	REAL8 values[LALSQTPN_OLD_NUM_OF_VAR], dvalues[LALSQTPN_OLD_NUM_OF_VAR];
	LALSQTPNIntegratorSystem integrator;
	xlalErrno = 0;
	if (XLALSQTPNIntegratorInit_Old(&integrator, LALSQTPN_OLD_NUM_OF_VAR, params, LALSQTPNDerivator_Old)) {
	...
	values[LALSQTPN_OLD_PHASE] = params->phi;
	values[LALSQTPN_OLD_OMEGA] = params->lowerFreq * freq_Step;
	values[LALSQTPN_OLD_LNH_1] = sin(params->inclination); ///< \f$\hat{L_N}=\sin\iota\f$
	values[LALSQTPN_OLD_LNH_2] = 0.; ///< \f$\hat{L_N}=0\f$
	values[LALSQTPN_OLD_LNH_3] = cos(params->inclination); ///< \f$\hat{L_N}=\cos\iota\f$
	values[LALSQTPN_OLD_MECO] = 0.;
	for (i = 0; i < 3; i++) {
		values[LALSQTPN_OLD_CHIH1_1 + i] = params->chih[0][i];
		values[LALSQTPN_OLD_CHIH2_1 + i] = params->chih[1][i];
	}
	...
	dvalues[LALSQTPN_OLD_MECO] = -1.; // to be able to start the loop
	i = 0;
	do {
		alpha = atan2(values[LALSQTPN_OLD_LNH_2], values[LALSQTPN_OLD_LNH_1]);
		amp = params->signalAmp * pow(values[LALSQTPN_OLD_OMEGA], 2. / 3.);

		// calculating the waveform components
		if (waveform->hp || waveform->hc) {
			temp1 = -0.5 * amp * cos(2. * values[LALSQTPN_OLD_PHASE]) * (values[LALSQTPN_OLD_LNH_3]
					* values[LALSQTPN_OLD_LNH_3] + 1.);
			temp2 = amp * sin(2. * values[LALSQTPN_OLD_PHASE]) * values[LALSQTPN_OLD_LNH_3];
			if (waveform->hp) {
		...
		if (waveform->waveform) {
			waveform->waveform->a->data->data[2 * i] = -amp * 0.5 * (1. + values[LALSQTPN_OLD_LNH_3]
					* values[LALSQTPN_OLD_LNH_3]);
			waveform->waveform->a->data->data[2 * i + 1] = -amp * values[LALSQTPN_OLD_LNH_3];
			waveform->waveform->phi->data->data[i] = 2. * (values[LALSQTPN_OLD_PHASE] - params->phi);
			waveform->waveform->shift->data->data[i] = 2. * alpha;
			waveform->waveform->f->data->data[i] = values[LALSQTPN_OLD_OMEGA] / freq_Step;
		}
		...
		if (isnan(values[LALSQTPN_OLD_PHASE]) || isnan(values[LALSQTPN_OLD_OMEGA]) || isnan(
				values[LALSQTPN_OLD_LNH_1]) || isnan(values[LALSQTPN_OLD_LNH_2]) || isnan(
				values[LALSQTPN_OLD_LNH_3]) || isnan(values[LALSQTPN_OLD_CHIH1_1]) || isnan(
				values[LALSQTPN_OLD_CHIH1_2]) || isnan(values[LALSQTPN_OLD_CHIH1_3]) || isnan(
				values[LALSQTPN_OLD_CHIH2_1]) || isnan(values[LALSQTPN_OLD_CHIH2_2]) || isnan(
				values[LALSQTPN_OLD_CHIH2_3])) {
			break;
		}
		LALSQTPNDerivator_Old(time, values, dvalues, params);
	...
	} while (dvalues[LALSQTPN_OLD_MECO] < 0. && dvalues[LALSQTPN_OLD_OMEGA] > 0.0 && SQT_SQR(
			values[LALSQTPN_OLD_LNH_3]) < 1. - LNhztol && values[LALSQTPN_OLD_OMEGA] / freq_Step
			< params->samplingFreq / 2./* && values[LALSQTPN_OLD_OMEGA] / freq_Step < params->finalFreq*/);
	if (waveform->hp || waveform->hc) {
		params->finalFreq = values[LALSQTPN_OLD_OMEGA] / (LAL_PI * geometrized_m_total);
