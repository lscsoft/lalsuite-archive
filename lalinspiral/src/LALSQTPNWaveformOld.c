...
int LALSQTPNDerivator_Old(REAL8 t, const REAL8 values[], REAL8 dvalues[], void * param) {
	...
	REAL8 alphadotcosi, LNhxy = SQT_SQR(values[LALSQTPN_LNH_1]) + SQT_SQR(values[LALSQTPN_LNH_2]);
	if (LNhxy > 0.0) {
		alphadotcosi = values[LALSQTPN_LNH_3] * (values[LALSQTPN_LNH_2] * dvalues[LALSQTPN_LNH_1]
				- values[LALSQTPN_LNH_1] * dvalues[LALSQTPN_LNH_2]) / LNhxy;
	} else {
		alphadotcosi = 0.0;
	}
